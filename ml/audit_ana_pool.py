...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06）—— **1.20倍は裾だった。中央値は1.000倍**
────────────────────────────────────────────────────────────
...
■ ★★★★決定打: **中央値の比が 1.000倍**
| 券種 | ⚠**算術の配当比** | ★**幾何（対数）** | ★★**中央値** | 対数の99%区間 | 判定 |
|---|---|---|---|---|---|
| 馬連 | 1.029倍 | 1.016倍 | **1.000倍** | [−0.004,+0.034] | ⚠通らない |
| 馬単1点 | 1.058倍 | 1.028倍 | 1.011倍 | [−0.003,+0.061] | ⚠通らない |
| 三連複 | 1.056倍 | 1.029倍 | 1.012倍 | [+0.002,+0.054] | ★有意 |
| ⚠**三連単** | ⚠**1.200倍** | ★**1.057倍** | ★★**1.000倍** | [−0.005,+0.118] | ⚠**通らない** |
（(0.20,0.06): 三連単 算術1.135 / 幾何1.031 / **中央値0.982倍**・[−0.030,+0.086] ⚠通らない）
★★★**普通に当たったときの配当は対照とまったく同じ**。**違うのは特大配当だけ**。
　→ ★★**1.20倍という数字はそこだけで作られていた**。**8比較中7つが通らない**。
★**事前登録の予想は半分当たり**（「差は残るが1.20より小さい」→ **小さくなり、有意でもなくなった**）。

■ ★★★★三連単の「信頼できるROI」
**103.5% ＝ 81.7%（帯）× 1.101（当てる腕前・★有意）× 1.200（配当の上乗せ・⚠裾）**
★**③を裾を除いた 1.057倍 に置き換えると**:
　**81.7% × 1.101 × 1.057 = ★95.1%**
★★**三連単の実力は約95%**。⚠**103.5%との差 8.4pt はまるごと裾**。

■ ★★★結論（**(197)(198)(199)の3本で決着**）
　★**「モデルは三連単でも良い馬を選んでいる」＝本当**。**頭数を増やすと増幅する**（1.062→1.100倍）。
　⚠**「だから三連単で勝てる」＝繋がらない**。**実力は約95%**。
　★**充足率は馬連82.3% / 三連単79.8%**＝**どの券種でも足りない量はほぼ同じ**。
　⚠**プールをまたぐ歪みの仮説は死んだ**——**中央値が1.000倍では、歪みとは呼べない**。
