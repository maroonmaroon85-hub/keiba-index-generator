...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-05・25,811レース）—— **主判定 10/10 陰性。だが得たものは大きい**
────────────────────────────────────────────────────────────

■ ゲート1: 4帯とも通過 ／ ★陽性対照: 三連複BOX上位4 = 既知84.5%を再現して立った

■ ★★★**ご指摘への回答: 「点数を増やすと悪化」は選別後でも成り立った**
| 選別 | 買い方（穴×人・三連複） | 点数 | ROI | 1R損益 |
|---|---|---|---|---|
| 全レース | 軸2頭 + モ1 | **1** | **83.1%** | −16.9円 |
| 全レース | 軸2頭 + モ3 | **3** | **72.1%** | **−83.7円** |
| **(112)裾2%** | 軸2頭 + モ1 | **1** | **97.2%** | **−2.8円** |
| **(112)裾2%** | 軸2頭 + モ3 | **3** | **62.0%** | ★**−113.9円** |
★**選別後のほうが悪化が激しい**（−11.0pt → **−35.2pt**）。**3頭目を1→3頭に増やしただけ**。
→ ★**(77)(171)(172)が全レースでしか測っていなかった点は本当の穴だったが、結論の向きは正しかった**。

■ ★2軸そのもの（**全レース 25,811R**・ROI%）
| 買い方 | 点数 | ★**穴×人** | 乱×人 | **モ2** | 人2 |
|---|---|---|---|---|---|
| 馬連 | 1 | **70.8** | 66.8 | ★**83.1** | 79.5 |
| ワイド | 1 | **73.6** | 67.8 | ★**82.6** | 78.9 |
| 馬単 両方向 | 2 | **61.8** | 59.6 | ★**79.6** | 76.2 |
| 三連複 +モ1 | 1 | ★**83.1** | 70.1 | 80.7 | 79.7 |
| 三連複 +モ3 | 3 | **72.1** | 70.5 | ★**81.7** | 81.3 |
★★**5つ中4つで「穴×人」が「モデル上位2頭」に負ける**。**人気上位2頭にも負ける**（採用条件2を満たさない）。
→ ★**2軸という形は、穴の弱さを救わなかった**。

■ ★★主判定: **10比較すべて99%CIが0を跨いだ**（最大 三連複+モ1・裾2% の +38.5円 [−107.5,+184.5]）

■ ★★★**この実行でいちばん価値のある発見: プラセボが109.9%を出した**
| 選別 | 買い方 | ★**乱×人（中身が無作為）** |
|---|---|---|
| (112)裾2% | **馬単 軸2頭 両方向** | ★★**109.9%（+19.9円）** |
| (112)裾2% | 馬連 軸2頭 | ★**99.6%** |
★**中身が完全に無作為な軸が、100%を超えるマスを2つ作った**。**(112)裾2%は397レース(1.5%)**で
　**CIは ±115〜±471円**。→ ★★**「最適な買い方」を最良のマスで判断すると、この2マスを掴む**。
　**事前登録で「最良のマスを選んで結論にしない」と縛っておいたのが効いた**。
　★**判定基準4（総当たりでは偶然だけで34.4セルが通過）の、この標本での実演**。
⚠**同時に、(112)裾2%のアームは検出力が無いことも確定**（**判定基準5「絞ると測れなくなる」の再現**）。
　**登録したこと自体は正しかったが、397レースでは何も読めない**。

■ ★橋渡し（**複勝1点・全レース**）—— **(177)と整合**
| 軸 | ROI |
|---|---|
| **モデル1位** | ★**85.1%** |
| 1番人気 | 83.9% |
| **穴（比が最大）** | **73.3%** |
| **乱（オッズ帯を揃えた無作為）** | **63.4%** |
★**穴 − 乱 = +9.9pt**＝**(177)の「オッズを中和しても +15.2円」と同じ向き**。**穴の選定に情報はある**。
⚠★**しかし「比が最大の馬」という穴の定義は悪い**——**モデル1位に11.8pt負ける**。
　**(171)で軸の平均オッズが18.6倍まで穴側に寄ったのと同じ**＝
　★**(75)(120)の「残差順は人気薄順になる」がそのまま出ている**。

■ ★★結論
　**「穴馬 × 1番人気」の2軸は、オッズ帯を揃えた無作為な穴に勝てない**（10比較とも検出できず）。
　**しかも モデル上位2頭 にも 人気上位2頭 にも負ける**。
　⚠**経路は弱いので「閉じた」とは書けない**（判定基準25）。
　★**書けるのは「モデルの穴の選定は、オッズ帯を揃えた無作為な穴に勝てない」まで**。
　★**副産物のほうが重い**: **プラセボが109.9%**／**点数の悪化は選別後でも成り立つ**／
　　**(112)裾2%は397レースで検出力ゼロ**。
