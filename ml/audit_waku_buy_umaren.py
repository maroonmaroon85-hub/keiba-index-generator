...
★★事前登録（**測る前に書いている**）
　1. **買い方は3つだけ**。**後から増やさない**:
　　 **A** ⚠**「現行(モデル)」と書いたが実際は市場順だった**（→**(153b)で訂正**）:
　　 　 `probs(hs)`＝**単勝オッズ由来**の1位馬の枠 × 2位馬の枠 **1点**
...
⚠⚠★★**この(153)には誤りがある（同日中に自分で見つけた）**——
　**Aを「現行(モデル)」と表示したが、中身は `probs(hs)`＝単勝オッズ由来の市場順**で、
　**MLモデルは1行も使っていない**。→ **(153)が比べたのは「市場順 vs 馬連プール」**。
　**本来の問い「モデル順 vs 馬連プール」は `ml/audit_waku_buy_ml.py`（153b）で測り直した**。
　★**陽性対照が 83.3% vs 既知85.2% で −1.9pt ずれたのがヒントだった**。
　**±3ptの許容に入ったので見逃した**。⚠**許容幅は「近い」を保証するが「同じ」は保証しない**。

★★★実行済みの結果（2026-08-16・34,160レース・2015年以降）**市場順と馬連プールに差は無い**
...
★★**結論: 馬連プールで買い目を選んでも市場順と全く同じ**。⚠**現行(モデル)との比較は(153b)**。
