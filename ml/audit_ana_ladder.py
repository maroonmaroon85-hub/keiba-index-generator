...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06）—— **増幅は「頭数」から。着順ではない。配当の上乗せは本物**
────────────────────────────────────────────────────────────

■ ゲート1 ／ ゲート板 0.8009 ／ 陽性対照 BOX4 81.9% ／
　★★内部対照（決定的）: **三連単 (0.15,0.02) ROI = 103.1%** → **完全再現**

■ ★★★家族A: **梯子は単調に上がった**（**8比較中7つが有意**）
| 段 | 券種 | 頭数 | 着順 | 本の的中率 | 乱帯 | ★★**比** | 必要 | ★**充足率** |
|---|---|---|---|---|---|---|---|---|
| 1 | 馬連 | 2 | 不問 | 11.377% | 10.713% | **1.062倍** | 1.290 | 82.3% |
| 2 | ★**馬単1点** | 2 | ★**要** | 5.812% | 5.404% | **1.076倍** | 1.333 | 80.7% |
| 3 | ★**三連複** | ★**3** | 不問 | 7.134% | 6.488% | ★**1.100倍** | 1.333 | 82.5% |
| 4 | 三連単 | 3 | 要 | 2.629% | 2.389% | **1.101倍** | 1.379 | 79.8% |
（(0.20,0.06)も同じ形: **1.059 → 1.076 → 1.088 → 1.083倍**。⚠**三連単だけ有意でない**）
★★★**読み取れること3つ**:
　★**頭数を2→3に増やすと +3.6%**（1.062 → 1.100）＝**利用者の言う「増幅」の正体**
　★**着順を要求しても +0.1%**（三連複 1.100 → 三連単 1.101）＝**着順は増幅を壊さない**
　★**着順の腕前は少しある**（馬連 1.062 → 馬単1点 1.076・**+1.3%**）
　　⚠**事前登録の予想「着順の腕前は無い」は外れた**（判定基準24）。**ただし小さい**
⚠**「必要な比」も一緒に上がるので、充足率は 80〜82% でほぼ一定**
　＝★★**どの段まで登っても、同じだけ足りない**。

■ ★★★★家族B: ★**配当の上乗せは対照の粗さではなかった**（**乱細が作れたのは99.2%**）
| 券種 | 配当比（乱帯） | ★**配当比（乱細・±20%）** |
|---|---|---|
| 馬連 | 1.03倍 | **1.03倍** |
| 馬単1点 | 1.06倍 | **1.06倍** |
...
★**事前登録の予想「粒度の産物」は外れた**（判定基準24・2つ目）。
★★**三連単だけ、同じオッズの馬を集めても配当が2割高い**
　＝**三連単プールは、単勝オッズが示す以上にこの馬たちを割安に扱っている**。
　★★**プールをまたぐ歪み——枠連117.3%を生んだのと同じ系統（ROI_MAP IV「比の裾」）**。
⚠**ただし774回の的中だけで決まる数字**。★**本物かはブートストラップで測る必要がある**（→(199)）。

■ ★★結論
　★**増幅は実在し、その源は「頭数」**。**着順は中立〜わずかに正**。
　⚠**だが充足率は80〜82%で一定＝券種を変えても足りない量は変わらない**。
　★★**三連単の103.1%を作っているのは「配当の上乗せ1.20倍」で、これは粒度の産物ではなかった**。
　→ ★**次: その1.20倍が統計的に本物かを測る**。⚠**そこが本物なら、系統IVの入口になる**。
