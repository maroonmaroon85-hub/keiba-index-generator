...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-05・18,800レース）—— **ゲート3で停止。だが落ち方が答えになった**
────────────────────────────────────────────────────────────

■ ゲート1: 4帯とも通過 ／ 陽性対照: 三連複BOX上位4 **83.1%** vs 既知84.5% → 立った
⚠**突き合わせが 26,583 → 18,800レースに減った**（穴＝1番人気の除外と、3-15倍に馬が
　居ないレースの除外）。**(174)〜(178)と完全に同じ標本ではない**。**水準の比較には注意**。

■ ★★ゲート3: **穴の定義は直ったか**（複勝1点）
| 軸 | 複勝ROI | 平均オッズ | 判定 |
|---|---|---|---|
| ★**穴A**（全帯・帯内パーセンタイル最大） | ⚠**70.8%** | ⚠**69.7倍** | ⚠⚠**落ちた**（要≥80.0%） |
| ★**穴B**（3-15倍に限定） | **80.6%** | **8.3倍** | ★**通った** |
| **穴C**（比が最大・(178)と同一） | **72.3%** | **133.9倍** | ★**内部対照 再現**（(178) 73.3%・±2pt内） |
| モデル1位 | ★**84.7%** | — | — |
| 人気上位2 | 83.5% | — | — |
| **乱A** | **71.4%** | — | ★**穴A − 乱A = −0.6pt** |
| **乱B** | ★**79.6%** | — | ★★**穴B − 乱B = +1.0pt だけ** |
| **乱C** | **64.8%** | — | ★**穴C − 乱C = +7.5pt** |
★**穴Aが落ちたのでゲート3は不成立**＝**事前登録どおり2軸を読まずに停止**。

■ ★★★**落ち方が、この線全体の答えになっている**
　★**穴A（全帯で帯内パーセンタイル最大）の平均オッズが 69.7倍**——
　　**帯内で正規化しても、5帯の最大値を取ると 50倍〜の帯から選ばれる**（**その帯が最も頭数が多い**）。
　　→ ★★**「帯内で切る」だけでは足りず、「どの帯から取るか」を固定しないと必ず穴に落ちる**。
　　　 **(171)の18.6倍・(178)の133.9倍・本件の69.7倍で3回目**。
　★★★**モデルの残余情報は「水準が絶望的な超人気薄」に集中している**:
| 穴の定義 | 平均オッズ | 穴 − 乱（**オッズを揃えた無作為との差**） | 水準 |
|---|---|---|---|
| **穴C** | **133.9倍** | ★**+7.5pt**（大きい） | ⚠**72.3%**（絶望的） |
| **穴A** | 69.7倍 | **−0.6pt**（無い） | ⚠70.8% |
| ★**穴B** | ★**8.3倍** | ⚠**+1.0pt だけ** | **80.6%**（払戻率ぎりぎり） |
　→ ★★**水準がまともな帯（3-15倍）では、モデルの穴の選定はほぼ何も足していない**。
　　 **大きな差が出るのは、そもそも張れない超人気薄の領域だけ**。
　★**これが(174)の +7.6pt と (177)の +1.5pt が食い違った理由**——
　　**(174)は「帯の上位10% vs 下位10%」の幅を見ていた**が、**(177)(179)は「無作為との差」を見ている**。
　　⚠**幅が大きいことと、無作為に勝つことは別**。

■ ★★結論
　★**穴の定義を(174)の形に取り直しても直らなかった**。
　　**帯を限定した穴B ですら、オッズを揃えた無作為に +1.0pt しか勝てない**。
