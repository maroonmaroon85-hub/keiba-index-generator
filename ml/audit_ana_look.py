...
**★★★⑤軸の人気順位（(239)のプロフィール文の検算・2026-09-09 追記）**
| 人気 | 本数 | 割合 | 中央オッズ |
|---|---|---|---|
| ⚠**1〜2番人気** | 5 | ⚠**0.4%** | 10.7倍 |
| 3番人気 | 40 | 2.9% | 11.6倍 |
| 4〜5番人気 | 403 | 28.8% | 12.3倍 |
| ★**6〜8番人気** | ★**711** | ★**50.9%** | 16.7倍 |
| 9番人気以下 | 239 | 17.1% | 33.8倍 |
　★★**3番人気以内は3.2%**（**1〜2番人気は0.4%**）→ ★**「◎はいつも人気薄」は96.8%正しい**。
　⚠★**軸の条件は「単勝≥10.0倍」であって人気順位ではない**——
　　**1番人気が極端に短いレースでは、上位人気でも10倍を超える**。★**そこが例外の正体**。
　★**「単勝10倍以上」の方は定義上100%正しい**。
...
def main():
    ...
    print(f"\n■ ★★★**⑤軸の人気順位の分布**　← ★**プロフィール「◎はいつも人気薄」の検算**")
    print("　⚠★**軸の条件は「単勝≥10.0倍」であって★人気順位ではない**"
          "——**1番人気が極端に短いレースでは、上位人気でも10倍を超える**")
    print(f"{'人気':<12}{'本数':>8}{'割合':>9}{'累積':>9}{'中央オッズ':>12}")
    cum = 0
    for lo, hi, lab in ((1, 2, "1〜2番人気"), (3, 3, "3番人気"), (4, 5, "4〜5番人気"),
                        (6, 8, "6〜8番人気"), (9, 99, "9番人気以下")):
        m2 = (rk >= lo) & (rk <= hi)
        cum += m2.sum()
        if not m2.sum():
            continue
        print(f"{lab:<12}{m2.sum():>8,}{100*m2.mean():>8.1f}%{100*cum/len(rk):>8.1f}%"
              f"{np.median(od[m2]):>11.1f}倍")
    p3 = 100.0 * np.mean(rk <= 3)
    print(f"\n　★**3番人気以内が {p3:.1f}%**　"
          + ("⚠**『いつも人気薄』とは書けない**" if p3 >= 10
             else "★**『いつも人気薄』はほぼ正しい**"))
    print(f"　★**単勝10.0倍以上は定義上100%**——★**そちらは常に正しい**")
