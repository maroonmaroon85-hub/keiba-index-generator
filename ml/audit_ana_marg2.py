...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-06・29,359レース）—— **結論は保った。だが22/22マスが改善した**
────────────────────────────────────────────────────────────

■ ゲート板: 復元R = **0.8009** → ★立った ／ 陽性対照: BOX4 **81.9%** → ★立った
■ ★内部対照: **モ1軸が全22マスで(182)を±2pt以内に再現** → ★**装置は同一**
■ 記述: **穴(板) と 穴(単勝) が同じ馬になる割合 = 65.9%**（**(184)は73.9%**・帯が狭いぶん低い）

■ ★★★主判定（★**穴(板)軸のみ**・22比較・α=0.01/22・z=3.506）
| 券種 | k | ★**穴(板)** | (182)穴(単勝) | 差 | 99%CI(Bonf) | 判定 |
|---|---|---|---|---|---|---|
| **馬単** | 2 | ★**93.1%** | 89.2% | +3.9 | [77.7,108.6] | ⚠100%を超えない |
| **馬連** | 2 | ★**92.8%** | 89.9% | +2.9 | [82.8,102.8] | ⚠100%を超えない |
| **三連単** | 3 | ★**91.0%** | 77.0% | ★**+14.0** | [72.3,109.7] | ⚠100%を超えない |
| 三連複 | 2 | 89.3% | 87.3% | +2.0 | [79.5,99.0] | ⚠100%を超えない |
| 馬単 | 5 | 89.4% | 83.8% | +5.6 | [65.1,113.7] | ⚠100%を超えない |
| 馬連 | 1 | 87.1% | 82.9% | +4.2 | [79.4,94.7] | ⚠100%を超えない |
| …（全22マス） | | | | | | |
★**22比較すべてで99%CI下端が100%に届かない（0マス）**。→ ★**(182)の結論は保った**。
...
| **中央** | +4.0pt |
| **範囲** | +0.9 〜 **+14.0pt** |
★**最良が 89.9% → 93.1% に上がった**。⚠**それでも100%には6.9pt足りない**。

■ ★★★**「板の価格が正しい」ことの独立な確認が3本揃った**
| | 確認 |
|---|---|
| **(184) ゲート板** | **復元R = 0.8009**（公示0.800・調和平均） |
| **(184) 主判定A** | **有意な帯 1/9 → 6/9**、**水準 84.9% → 88.4%** |
| ★**(185)** | ★**22/22マスが改善**（平均+4.7pt・**符号がひとつも逆にならない**） |
★**符号が22/22で揃うのは偶然では起きにくい**（**両側符号検定 p = 2×0.5²² ≈ 4.8e-7**）。
　⚠**post-hocなので結論には使わない**が、**価格の差し替えが実質的だったことの傍証**。

■ ★★結論
　★**穴の価格を板に直しても、紐を1頭足す増分は常に損**。**「組を買うこと自体が損」は保った**。
　★**だが「どれだけ損か」は一貫して 約4.7pt 過大評価していた**——
　　**(182)の数字をそのまま引用してはいけない**。
