...
⚠★**`norm_horse` はここに置く**（2026-09-06）——**`nk_link.py` が pandas 無しで読む**ため。
　**一度 `features.py` に置いて `nk_link` から import し、Macで ModuleNotFoundError にした**。
　★**`nk_link.py` は「標準ライブラリだけで動く」という決めごとがある**。**破らないこと**。
"""
import csv
import html as H
import json
