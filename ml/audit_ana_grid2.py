...
■ ★★★**判定不能マスの決着（判定基準26「物差しを替えられないか考える」を当てた）**
　★**三連単(0.15,0.02) k=2 = 103.1% / 99%CI [72.1,134.1] / 半幅 ±31.0pt / 的中率2.63%**。
| | |
|---|---|
| 下端を100%超にするのに要る半幅 | **±3.1pt 未満** |
| **必要な標本** | ★**現在の100倍 = 2,943,700レース** |
| JRAは年 約3,183レース | → ★★**925年分** |
　→ ★★**原理的に測れない**（**判定基準5「絞ると測れなくなる」の極端な例**）。
　★**そして他の41マスは全部、点推定が100%未満**（三連単(0.20,0.06) 96.6% / 馬単 91.9% /
　　三連複 91.2%）——**標本を増やしても届かない**。
　→ ★★★**42マス全部が「点推定が100%未満」か「925年必要」のどちらか**。**これで完全に閉じた**。
　⚠**物差しを替える案も検討したが使えない**——**対応差（穴軸 vs 乱軸）にすれば払戻の
　　ばらつきは消せるが、それは「無作為な軸に勝つか」の問いで、「100%を超えるか」ではない**。

■ 記述: **(189)との標本数の違い**（**確認済み・バグではない**）
　**(189)の(0.15,0.02)は 21,273R、(190)は 29,437R**。★**差は「乱が同じ帯に居ること」の要求**——
　**(189)は対照が要るので穴が帯に単独のレースを落とし、(190)の複勝行は落としていない**。
　★**それでも複勝ROIは 91.5% vs 91.5% で一致**＝**この脱落は穴の成績に対して情報を持たない**。
