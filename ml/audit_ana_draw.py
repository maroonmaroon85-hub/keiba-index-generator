...
────────────────────────────────────────────────────────────
★★★**内部対照は一致**（年別レース数が (236)(240)(241) と完全一致）。
★★**ゲート2も通過**（**① がゼロを含む＝pnが馬番を知らないという前提は保たれている**）。

| | 2016 | 2018 | 2020 | 2022 | 2024 | 2026 | ★**全期間** | ★**99%CI** |
|---|---|---|---|---|---|---|---|---|
| ★**① corr(align, pn)** | −.0004 | −.0000 | −.0059 | +.0025 | +.0037 | +.0124 | **+0.0025** | [−0.0015, +0.0063] ★**含む** |
| ★**② corr(align, qp)** | −.0023 | −.0009 | −.0044 | −.0004 | +.0047 | +.0130 | **+0.0019** | [−0.0024, +0.0060] ★**含む** |
| ★★**③ corr(align, gap)** | +.0057 | +.0037 | −.0039 | +.0089 | −.0022 | +.0019 | ★**+0.0018** | ★**[−0.0017, +0.0060] 含む** |
| ⚠**④ 軸のalign平均** | +.0634 | +.0684 | +.0208 | +.0127 | +.0979 | −.0268 | ⚠**+0.0561** | ⚠**[+0.0376, +0.0756] 外す** |
　★**整合**: **③ ≈ ① − ②** → **+0.0018 ≈ +0.0025 − (+0.0019)**　★**合う**。
　★**軸の本数は 1,398**（**年別 360/233/219/161/98/87/62/43/57/41/37**）。

■ ★★★★**主判定の返り値: ズレは馬番の関数を含んでいない**
　★**③ の99%CIがゼロを含む**。→ ★★**「ズレの一部がモデルの欠落では」という心配は、
　　★馬番に関しては否定された**。
　★★**理由は ② にある——★市場も馬番に値段を付けていない**（**②もゼロを含む**）。
　　→ ★**`pn − qp` に馬番の成分が乗りようがない**。

■ ⚠★★**ただし ④ が残った: 軸は外枠に偏っている**
　★**+0.0561＝平均で頭数の5.6%ぶん外寄り**。★**99%CIがゼロを外す**。
　★★**③がゼロなのに④が偏るのは、★ズレ経由ではないから**。
　　→ ★**「単勝10.0倍以上」という条件が外枠を拾っている**と読むのが自然
　　　（★**外枠不利なら人気が落ちてオッズが上がる**）。⚠**この機構は測っていない**。
　→ ★**軸は構造的に外枠寄り**。**枠連側(177)の内外バイアスが実在するなら、★軸は受ける側にいる**。
　⚠★**これは「バイアスで絞れば良くなる」を意味しない**。★**絞りは測っていない**。

■ ⚠**予想は外れた（★13回目）**
　★**「②③ともゼロを外すと見る」と書いたが、★両方ゼロを含んだ**。
　★**市場が枠順に値段を付けているはず、という見立てが違った**。

■ ⚠**先に書いた限界のとおり**
　★**これは「絞ればROIが動くか」ではない**。★**絞りを測るなら別に事前登録が要る**
　　（**統計量はROI・★同じ本数を無作為に捨てるプラセボ必須**——**枠連側(178)の②が壊れた理由**）。
　★**内外だけ。前残りは測っていない**。★**日ごとの馬場差も見ていない**（**通年の平均のみ**）。
　⚠**枠連側(177)(178)の結論は持ち込んでいない**。
