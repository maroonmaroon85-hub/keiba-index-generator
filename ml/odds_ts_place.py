...
def ceiling(races, rng):
    ...
    # 最良の帯が「選んだから高い」だけなのかを年代で割って確かめる（判定基準6）
    print(f"\n{'帯':<12}{'〜2019':>18}{'2020〜':>18}  ← 最良帯が年代で保つか")
    for lo, hi in [(1.0, 1.3), (blo, bhi), (1.0, 1.6)]:
        cells = []
        for a, b in [(2013, 2019), (2020, 2026)]:
            v = [fuku_pay(r, u) for r in races if r["fuku"] and a <= r["year"] <= b
                 for u, o, _f in r["horses"] if lo <= o < hi]
            cells.append(f"{len(v):,}頭 {np.mean(v):.1f}%" if v else "—")
        print(f"{f'{lo}-{hi}倍':<12}{cells[0]:>18}{cells[1]:>18}")
    print("　→ ★**最良帯は年代で大きく動く＝選んだから高いだけ**。年代で安定しているのは")
    print("　　 ≤1.3倍の**97%前後**の方で、これが実質的な天井。いずれにせよ100%に届かない。")
