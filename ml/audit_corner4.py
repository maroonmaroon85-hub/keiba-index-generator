...
■ ゲート: **検証AUC 0.8027 ＝ (174-枠) と完全一致**（★装置は同じ）。主判定を読んだ。
■ ★★主判定（検証 408,834行・logit(p) を差し引いた後の偏相関）

| | 偏相関 | 99%CI |
|---|---|---|
| ★**本 prev_res（4角残差）** | **+0.0008** | **[−0.0033, +0.0048]** ⚠**ゼロを跨ぐ** |
| **対照 prev_ra（モデル内）** | +0.0012 | [−0.0029, +0.0052] ⚠**ゼロを跨ぐ** |

　★**①本が対照を上回るか → いいえ**（0.0008 vs 0.0012）。
　★**②年別の符号がそろった年 7/10 → 満たさない**（必要 8/10）。
■ 参考（偶数年で学習・奇数年で検証）: 本 +0.0040 / 対照 +0.0034。**どちらもゼロを跨ぐ**。
　★**時間順と偶奇で同じ結論**＝**時代の変化の産物ではない**。

★★★**結論: モデルが既に拾っている。この線は閉じる。絞りは作らない**（判定基準40）。

⚠⚠★★**この測定でいちばん学ぶところ**——**対照を一段弱く取ると、消える信号が生き残って見える**。
　**前走の平均位置と log単勝 だけを差し引いた段階では −0.0943・99%CI[−0.0965,−0.0921]**で、
　**n=1,369,961 なので「ゼロを外す」と自信を持って書けた**。
　★**モデルの `p` に差し替えた瞬間 +0.0008 になり、対照（モデルに確実に入っている変数）にも負けた**。
　→ ★**「Xは情報を足すか」を測るときの対照は、★いま動いている系そのもの**でなければならない。
　　 **その代理（特徴量を2つ手で選ぶ等）では足りない**。⚠**判定基準45の系**。
