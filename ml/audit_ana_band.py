...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06）—— **「ズレが大きいほど良い」は成り立っていた。私の説明が誤り**
────────────────────────────────────────────────────────────

■ ゲート1 ／ ゲート板 0.8009 ／ 陽性対照 BOX4 81.9% ／
　★★内部対照（決定的・従来の軸）: **三連単k=2 ROI = 103.1%** → **完全再現**（29,437R）

■ ★★★★家族A: **5帯すべて有意、しかも完全に単調**
| ズレの帯 | R数 | 買う率 | 軸の平均ズレ | 軸オッズ | ★**複勝ROI** | 乱 | ★★**対応差** |
|---|---|---|---|---|---|---|---|
| [0.02,0.04) | 13,097 | 44.4% | 0.030 | 12.1倍 | 84.1% | 78.3% | **+5.8円** |
| [0.04,0.06) | 11,456 | 38.9% | 0.050 | 12.6倍 | 87.7% | 79.6% | **+8.1円** |
| [0.06,0.10) | 13,146 | 44.6% | 0.078 | 11.9倍 | 89.3% | 80.0% | **+9.3円** |
| [0.10,0.15) | 8,128 | 27.6% | 0.120 | 11.9倍 | 91.8% | 78.3% | **+13.6円** |
| ★**[0.15,∞)** | 3,631 | **12.3%** | 0.187 | 10.5倍 | ★★**97.3%** | 79.5% | ★★**+17.9円** |
★★★**+5.8 → +17.9円 と単調に増え、5/5がBonferroniを通る**。
⚠**事前登録の予想「中間の帯が最良」は外れた**（判定基準24・★**3回連続で外している**）。

■ ⚠★★**私の説明の誤りを訂正する**
　⚠**(201)で「床を上げると後半が悪化」と見て、「ズレが大きいほど良いは成り立っていない」と
　　利用者に説明した**。★**それは誤読だった**。
| | 何が起きていたか |
|---|---|
| **(201)** | ⚠**軸は常に「ズレ最大の馬」**。**床を上げても落ちるのは6%のレースだけ**で、
　　　　　　　★**軸のズレはほとんど変わっていなかった** |
| ★**(203)** | ★**帯で強制するので、軸の平均ズレが 0.030 → 0.187 と本当に動く** |
...
★★**まったく別の軸の選び方で同じ97%台**。**この線の複勝の最高値**。
⚠**ただし買う率は12.3%（8レースに1回）**。**(191)でズレ床0.20は95.1%に落ちるので、
　★0.15付近が頂点**。

■ ⚠家族B（三連単）: **読めない**
| 帯 | 前半 | 後半 | 縮み | 的中率 | 乱前半 | 乱後半 |
|---|---|---|---|---|---|---|
| [0.02,0.04) | 70.2% | 66.9% | +3.3pt | 1.32% | 76.1% | 70.6% |
| [0.04,0.06) | 85.7% | **107.2%** | −21.5pt | 1.50% | 73.5% | 77.5% |
| [0.06,0.10) | 64.4% | 73.0% | −8.6pt | 1.64% | 57.8% | 76.2% |
| [0.10,0.15) | ★**151.1%** | **104.2%** | ⚠**+47.0pt** | 2.13% | ⚠**98.1%** | 86.5% |
| [0.15,∞) | 143.9% | 81.6% | ⚠**+62.3pt** | 2.75% | 96.0% | 78.2% |
★**前半で選ばれた [0.10,0.15) の後半は104.2%**、⚠**だが99%CIが [46.3,162.0]**。
⚠⚠**本の縮み47.0pt に対しプラセボは11.6pt**＝**採用条件4は満たされない**。
★**帯で軸を選ぶと三連単の的中率が 2.63% → 1.3〜2.8% に落ちる**
　（**推奨度最大＝人気馬寄りになり配当も下がる**）。→ ⚠**三連単には向かない選び方**。

■ ★★結論（**採用条件 1/4**）
　★★**家族Aは完全に成功**——**5/5有意・単調・機構は「ズレが大きいほど強い」**。
　⚠**採用条件2は満たさない**（**最大の帯が最良＝「大きいほど良い」の否定にならなかった**）。
　⚠**家族Bは条件3・4とも満たさない**（**三連単には帯の軸は向かない**）。
　★★**実用上の意味**: **複勝で 97.3%**（**買う率12.3%**）＝**この線の到達点**。
　　⚠**それでも100%には2.7pt足りない**。
