...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06）—— **103.1%は「2019年」と「選択バイアス」の合計だった**
────────────────────────────────────────────────────────────

■ ★★内部対照（決定的・制限なしの母集団）: **(0.15,0.02) 三連単k=2 = 103.1%** → **完全再現**
　★**制限あり母集団は105.0%**（**866R少ない・+1.9pt**）＝**母集団差を明示**（判定基準25）

■ ★★★★家族A: **前半で選び、後半で1回だけ試した**
| | 前半で選ばれた構成 | 前半 | ★**後半（1回だけ）** | 99%CI | ★**縮み** |
|---|---|---|---|---|---|
| ★**本** | 推奨度≥0.15 / ズレ≥**0.06** / 紐2頭 / 三連単 | **117.7%** | ⚠**92.0%** | [65.6,118.3] | ⚠**+25.8pt** |
| **プラセボ** | 同左 / 紐3頭 | 95.4% | 83.1% | [72.3,93.9] | **+12.3pt** |
⚠⚠**本の縮み(25.8pt)がプラセボの縮み(12.3pt)より大きい**＝★**採用条件2は満たされない**。
→ ★★**「117.7%は選択バイアスで説明できる」側の答え**。

■ ★★★★家族B: ★**ズレ床の最適が逆転した**（**利用者の問い「0.02は最適か」への直接の答え**）
**三連単・紐2頭の6通り**:
...
| 0.20 | **0.02** | 107.0% | **91.0%** |
| 0.20 | 0.04 | 107.1% | 90.1% |
| 0.20 | 0.06 | 109.1% | 88.2% |
★★★**答え: 0.02 が最良**。**両方の推奨度で、床を上げるほど後半が悪化する**。
⚠**前半では逆だった**（0.06が最良）。★**そこが選択バイアスの正体**——
　**前半で「0.06が良い」と見えたのは偶然で、後半では最悪だった**。
...
★★**まったく違う手続きが95%で一致**。⚠**偶然の一致とは考えにくい**。

■ ★★良い知らせ（**再現性はある**）
　★**前半と後半の順位相関 +0.600**＝**「どの買い方が良いか」には再現性がある**
　★**三連単・紐2頭が前半上位6つを独占し、後半でも88〜96%**（**他券種は86〜91%**）
　★**選ばれた構成の後半 92.0% > 36通りの平均 89.1%**＝**選ぶこと自体に +2.9pt の価値**
　★**プラセボは後半83.1%**——**本の95.8%とは12pt離れている**＝**モデルの優位は後半でも生きている**

■ ★★★結論（**採用条件 2/4**）
| ★満たした | ⚠満たさない |
|---|---|
| **3. 順位相関が正**（+0.600） | **1. 後半の下端が100%超**（92.0%・CI [65.6,118.3]） |
| **4. 選ばれた後半が平均を上回る**（+2.9pt） | ⚠⚠**2. 本の縮みがプラセボより小さい**（25.8 vs 12.3pt・**逆**） |
　★★**買い方の選び方は正しい（再現性あり）。モデルの優位も後半で生きている**。
　⚠**だが水準は 95〜96% で100%に届かない**。
　⚠⚠**103.1% は「2019年」((200)) と「選択バイアス」((201)) の合計だった**。
　★**利用者の「超える想定で進めたい」に対しては、★この後半95.8%が最も正直な推定値**。
