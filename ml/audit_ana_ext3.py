...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06）—— **追わない、と決められた**
────────────────────────────────────────────────────────────

■ ゲート1 4帯とも通過 ／ ゲート板 復元R 0.8009 ／ 陽性対照 BOX4 81.9% → 全部立った
■ ★★内部対照: **(0.15,0.02) C=0.04 = 102.2%（差 -0.0pt）／ (0.20,0.06) C=0.04 = 103.7%（-0.0pt）**
　→ **(192)を完全再現**。**装置は同一**。

■ ★★★家族A: **単調は 0.06 まで続き、0.08 で崩れた**——**0/8 通過**
| 絞り | 紐の床 | R数 | 買う率 | ズレ順ROI | 99%CI | 的中率 | 現行 | 対応差 |
|---|---|---|---|---|---|---|---|---|
| (0.15,0.02) | 0.04 | 12,644 | 42.9% | 102.2% | [80.8,123.6] | 10.05% | 89.7% | +12.4円 |
| (0.15,0.02) | 0.06 | 3,991 | 13.5% | ★**104.8%** | [67.4,142.3] | 11.15% | 91.1% | +13.7円 |
| (0.15,0.02) | 0.08 | 1,016 | 3.4% | ⚠**94.2%** | [43.1,145.3] | 13.48% | ⚠**105.2%** | ⚠**−11.0円** |
| (0.20,0.06) | 0.04 | 11,827 | 40.1% | 103.7% | [82.5,124.8] | 10.62% | 88.2% | +15.4円 |
| (0.20,0.06) | 0.06 | 3,966 | 13.5% | ★**106.8%** | [69.0,144.6] | 11.32% | 89.9% | +16.9円 |
| (0.20,0.06) | 0.08 | 1,017 | 3.5% | ⚠**94.1%** | [43.0,145.1] | 13.47% | ⚠**103.6%** | ⚠**−9.5円** |
| 両方 | 0.10 | 236 / 237 | — | ⚠**標本不足で判定しない** | | | | |
★**床0.06までは単調**（102.2→104.8 / 103.7→106.8%）。**2絞りで揃っている**。
⚠**床0.08で両方とも94%に落ち、同時に「現行(p降順)」が105.2 / 103.6%に跳ねる**——
　★★**現行はここまで88〜91%で床にほとんど反応しなかった**。**それが急に跳ねるのは雑音**
　（**1,016レース・的中率13.5%＝約137本の的中で全部が決まる**）。**判定基準5**。
⚠**CIは床を上げるほど広がる**（±22 → ±38 → ±51pt）。**標本の減りが水準の伸びより速い**。

■ ★★★家族B: ★**追わない、と数字で決められた**（判定基準26・(190)と同じ手続き）
| 絞り | 床 | ROI | R数 | レース/年 | 必要R数 | ★**必要年数** |
|---|---|---|---|---|---|---|
| (0.15,0.02) | 0.04 | 102.2% | 12,644 | 1,149 | 1,236,294 | ⚠**1,076年** |
| (0.15,0.02) | 0.06 | 104.8% | 3,991 | 363 | 238,356 | ⚠**657年** |
| (0.20,0.06) | 0.04 | 103.7% | 11,827 | 1,075 | 393,750 | ⚠**366年** |
| (0.20,0.06) | 0.06 | ★106.8% | 3,966 | 361 | 123,427 | ⚠**342年** |
★★**最良でも342年**。⚠**(190)の三連単925年より短いが、桁は同じ**。
→ ★★★**「標本を増やす」道は閉じた**。**この水準を有意にする方法は存在しない**。

■ ★★★★家族C: ★**物差しを替えたら通った**——**2/8**
| 絞り | 床 | R数 | **ズレ順の紐 複勝** | **現行の紐 複勝** | ★**対応差** | 99%CI |
|---|---|---|---|---|---|---|
...
★★**床0.04の2マスが Bonferroni を通った**（**16比較・z=3.421**）。**隣接(0.06)も同符号・同程度**。
★★★**「ズレ順は p降順より良い紐を選ぶ」は本当だった**——**馬連の裾を使わない物差しで確認できた**。
⚠**床0.08では差が消える**——**家族Aの崩れと同じ場所**。**標本の問題**（1,016R）と読める。
⚠**水準は 88.1〜88.5% で、複勝の払戻率80.0%は超えるが100%には届かない**。

■ ★★結論
　★★**機構は確立した。水準は測れない**——**この2つを別々の物差しで同時に示した**。
　★**紐の選び方（ズレ順）に情報があるのは家族Cで確定**（**複勝で +6.4円・2マス通過**）。
　⚠**馬連103.7 / 106.8% は、有意にするのに342年**。**★追わない**。
　⚠**床0.08の「現行105.2%」は、この母集団が雑音で動く証拠**——**深追いは危険**。
　⚠**経路は弱いので「閉じた」とは書けない**（判定基準25）。

■ ★この線で残っている道（**「標本を増やす」以外**）
　★**分散を下げる**: **家族Cの形（紐を複勝で測る）は通った**。⚠**だがそれは券種が違う**。
　★**軸そのものを強くする**——**ANA_TRACK §4d の「モデルの形が最適でない可能性」**。
　　⚠**今はやらない**（**利用者の指示**）。
