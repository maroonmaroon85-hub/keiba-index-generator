...
■ ★★★**事前登録の再修正（2026-09-06・★家族Bの結果はまだ一度も見ていない）**
　⚠⚠**第2版も内部対照が落ちた**（**前半 +20.3円 vs +23.6円**・許容±3.0円を0.3円超過）。
　★★**同じ誤りを2回繰り返した**——**モンテカルロで揺れる量に、揺れ幅より狭い許容を置いた**。
　★★★**根本的に直す: 内部対照を「決定的な量」に移す**。
　　**1. ★主腕(乱を使わない)の ROI 全期間/前半/後半 = 103.7 / 115.0 / 88.0% を ±0.5pt で再現**
　　　（**これは乱を1粒も使わないので厳密に再現するはず**——**装置が同一かどうかを直接測る**）
　　**2. ★乱まわりは「±◯円」ではなく、★20種で実測した幅に入るかで見る**
　　　**乱の平均ROI ∈ [82.9, 97.4]% ／ 前半の差 ∈ [+15.6, +32.5]円 ／
　　　　後半の差 ∈ [−16.0, +10.5]円**（**すべて `ml/audit_ana_seed.py` の実測値**）
　★**発明した許容ではなく、測った分布を使う**（判定基準38: **測れる定数は測る**）。
...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06・第3版）—— **仮説は死んだ。分布の縮小は原因ではない**
────────────────────────────────────────────────────────────

■ ゲート1 4帯とも通過 ／ ゲート板 0.8009 ／ 陽性対照 BOX4 81.9%
■ ★★内部対照（**2つとも立った**）
　★1（決定的）: **主腕ROI 103.7 / 115.0 / 88.0% を完全再現**（差0.0pt）
　★2（20種の実測幅）: **乱ROI 92.5%∈[82.9,97.4] / 前半差 +20.3円∈[+15.6,+32.5] /
　　後半差 −1.4円∈[−16.0,+10.5]** → **全部入った**

■ ★★★家族A: ★**ズレの分布は本当に縮んでいた**（**仮説は生きた**）
| 年 | gap上位2% | 上位5% | 上位10% | pn上位10% | 絶対閾値を満たす馬 | その平均オッズ |
|---|---|---|---|---|---|---|
| 2016 | **0.1732** | 0.1246 | **0.0837** | 0.5034 | **12.91%** | **10.4倍** |
| 2020 | 0.1358 | 0.0976 | 0.0654 | 0.5033 | 10.02% | 8.7倍 |
| 2023 | 0.1324 | 0.0928 | 0.0612 | 0.5118 | 9.52% | 7.3倍 |
| 2026 | ⚠**0.1255** | 0.0872 | ⚠**0.0575** | 0.5109 | ⚠**8.79%** | ⚠**8.6倍** |
★★**ズレの分布は11年で 28〜31% 縮んだ**（上位2% 0.173→0.126 / 上位10% 0.084→0.058）。
★**推奨度(pn)の分布は横ばい**（0.503→0.511）＝**縮んでいるのはモデルと市場の食い違いだけ**。
→ ★**固定閾値0.06は2016年には「上位7%」、2026年には「上位9%」を意味していた**。
...
| 上位5% | 上位10% | 上位20% | 3,422 | 103.5% | 134.2% | ⚠80.6% | −5.2円 |
| 上位5% | 上位20% | 上位20% | 4,475 | 103.2% | 127.8% | ⚠83.4% | −0.6円 |
| 上位10% | 上位10% | 上位20% | 6,236 | 98.4% | 116.9% | ⚠84.3% | +2.1円 |
| **上位10%** | **上位20%** | **上位20%** | 8,229 | 98.9% | 108.6% | **91.3%** | +7.9円 |
★★★**12マス全部で前半→後半が落ちる**。**1つも止まらなかった**。
→ ★★**仮説は死んだ**。⚠**原因は「ズレの分布が縮んだこと」ではない**。
★★**分布は確かに縮んでいたが、それを補正しても効果は戻らない**
　＝**「食い違いの大きさ」ではなく「食い違いの中身（価値）」が失われている**。
★**もう1つ**: ⚠**絞りがきついマスほど落差が激しい**
　（上位2%: 179.3→73.9＝**41%** ／ 上位10%: 108.6→91.3＝**84%**）。
　★**きつく絞った前半の高い数字ほど、後半に残っていない**——**(178)のプラセボと同じ匂い**。

■ ★★結論
　★**家族Aは生き、家族Bで死んだ**——**分布の縮小は実在するが、劣化の原因ではない**。
　⚠**残る説明は2つ**: **(a) 市場が賢くなり、モデルの残余情報そのものが減った**
　　／ **(b) 前半の高い数字が最初から裾の運だった（劣化ではなく回帰）**。
　★**(b)なら「昔は効いた」も怪しくなる**。★**この2つは分けて測れる**（次の課題）。
...
KNOWN_ROI = (103.7, 115.0, 88.0)               # ★★主腕(決定的)の全期間/前半/後半
ROI_TOL = 0.5
SEED_RNG = {"roi": (82.9, 97.4), "h1": (15.6, 32.5), "h2": (-16.0, 10.5)}  # ★20種の実測幅
KNOWN_H2 = -0.8                                # ★採用条件の比較基準(20種平均)
...
def selftest():
    ...
    print(f"★★内部対照1（決定的）: **主腕ROI {KNOWN_ROI} を ±{ROI_TOL}pt で再現**")
    print(f"★★内部対照2（実測幅）: **乱ROI∈{SEED_RNG['roi']}% / "
          f"前半差∈{SEED_RNG['h1']}円 / 後半差∈{SEED_RNG['h2']}円**（20種の実測）")
...
def main():
    ...
    got = (roi_of(z_), roi_of(z_[h]), roi_of(z_[~h]))
    ok_a = all(abs(a - b) <= ROI_TOL for a, b in zip(got, KNOWN_ROI))
    rr = roi_of(r_)
    inr = lambda v, k: SEED_RNG[k][0] <= v <= SEED_RNG[k][1]
    ok_b = inr(rr, "roi") and inr(d1, "h1") and inr(d2, "h2")
    ok1 = ok_a and ok_b
    print(f"\n■ ★★内部対照（**絶対閾値 {ABS_FILT}・床{ABS_C}**・{len(z_):,}R）")
    print(f"　★1（決定的・乱を使わない）: 主腕ROI "
          f"{got[0]:.1f} / {got[1]:.1f} / {got[2]:.1f}% vs "
          f"{KNOWN_ROI[0]} / {KNOWN_ROI[1]} / {KNOWN_ROI[2]}%"
          f"　→ **{'★再現' if ok_a else '⚠⚠ズレた'}**")
    print(f"　★2（20種の実測幅に入るか）: 乱ROI {rr:.1f}%∈{SEED_RNG['roi']} / "
          f"前半差 {d1:+.1f}円∈{SEED_RNG['h1']} / 後半差 {d2:+.1f}円∈{SEED_RNG['h2']}"
          f"　→ **{'★入った' if ok_b else '⚠⚠外れた'}**")
    if not ok1:
