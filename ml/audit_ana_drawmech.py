...
────────────────────────────────────────────────────────────
★★★★ 結果（2026-09-16）—— ⚠★★**(245)④は私の誤読だった。訂正する**
────────────────────────────────────────────────────────────
★★★**内部対照は2つとも立った**（年別レース数一致・⑤軸 +0.0561 / 1,398本が(245)④と一致）。

| | 頭数 | ★**align平均** | 99%CI |
|---|---|---|---|
| ★**⓪ 全馬（基準線）** | 408,271 | ★**+0.0451** | [+0.0440, +0.0463] |
| ★**① 単勝≥10.0倍だけ** | 289,123 | **+0.0442** | [+0.0429, +0.0455] |
| **② pn≥0.15だけ** | 204,918 | +0.0459 | [+0.0445, +0.0476] |
| **③ ズレ≥0.15だけ** | 6,738 | +0.0537 | [+0.0448, +0.0635] |
| **④ 3条件すべて** | 1,423 | +0.0559 | [+0.0361, +0.0719] |
| ★★**⑤ 軸（pn最大）** | 1,398 | **+0.0561** | **[+0.0360, +0.0743]** |

■ ⚠★★★★**最重要: `align` の全馬平均は解析的に `1/(2n)` で★常に正**
```
mean(align) = (1/n)Σ(i/n − 0.5) = (n+1)/(2n) − 0.5 = ★1/(2n)
　8頭 +0.0625 ／ 10頭 +0.0500 ／ 12頭 +0.0417 ／ 14頭 +0.0357 ／ 18頭 +0.0278
```
★**実測の⓪ +0.0451 は頭数11.1頭相当**。★★**つまりゼロではなく⓪が基準線**。
⚠★**(245)④で「軸のalign平均がゼロを外す＝軸は外枠に偏っている」と書いたのは誤り**。
　★**比べる相手はゼロではなく⓪だった**。

■ ★★★★**主判定の返り値: ① が ⑤ を説明できる（99%CIが重なる）**
　★**だが本当の答えはもっと単純**——★**⑤ +0.0561 と ⓪ +0.0451 の差は +0.0110 で、
　　★⑤のCI [+0.0360,+0.0743] が ⓪の +0.0451 を含む**＝★★**区別できない**。
　→ ★★**軸に枠順の偏りは無い**。★**①②③④のどれも⓪とほぼ同じ**。

■ ★**オッズ帯ごとの align（★「人気薄ほど外枠」は支持されない）**
| 帯 | [1,3) | [3,5) | [5,10) | [10,20) | [20,50) | [50,∞) |
|---|---|---|---|---|---|---|
| **align** | +.0533 | +.0481 | +.0444 | +.0445 | +.0412 | +.0459 |
　⚠**オッズと単調な関係が無い**。★**上下は頭数構成の違い**（`1/(2n)`は小頭数ほど大きい）で説明がつく。

■ ⚠**予想は外れた（★14回目）**
　★**「①がほぼ全部を説明する」と書いたが、★そもそも説明すべき偏りが無かった**。
　⚠★**「②③はゼロ近傍」も外れ**——**ゼロ近傍ではなく⓪近傍だった**（**物差しの中心を取り違えていた**）。

■ ★★**この測定が返したもの**
　★**トラックバイアスは「軸の選ばれ方」には入り込んでいない**。
　★**(245)③（ズレは馬番の関数を含まない）とあわせて、★枠順は軸にも妙味にも効いていない**。
　⚠**「外枠が有利／不利か」自体は測っていない**（★枠連側(177)がそちらを測っている）。
