...
★★★実行済みの結果（2026-08-14・1開催日36R→突き合わせ27R）**判定不能**
　配管は全部通った: **4時点すべてにオッズあり**／**確定 vs netkeibaの板 のずれ 0/899組・0.0000%**。
　(A)直前 ROI 39.7% [−60,139]・再現率27.3%・適合率26.5%・**D −0.0497**。
　⚠★★**事前登録の読み分け（直前でDが負なら閉じる）を当ててはいけなかった**:
　　**同じ表の「確定」列は真値既知**（(141)で31,130R・**+0.0266**）なのに **−0.0249** と符号を外した。
...
def main():
    ...
    print("★経路: q = **馬連の板→枠（厳密・Harville不使用）** / q_pool = **枠連の板**")
    print("　★**強い経路**（別の市場の意見を厳密に集約）。(141)と同じ土台。")
