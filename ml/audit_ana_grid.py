...
────────────────────────────────────────────────────────────
★★★★ 実測（2026-09-06・29,465レース）—— **この線で初めて採用条件を全部通った。だが水準は届かない**
────────────────────────────────────────────────────────────

■ ゲート1 4帯とも通過 ／ ゲート板 復元R **0.8009** ／ 陽性対照 BOX4 **81.9%** → 全部立った

■ ★★全16マス（**穴の複勝ROI**・カッコは平均オッズ）
...
■ ★★★主判定: **B統計（穴 − 乱）が 16/16マスで通過**（α=0.01/32・z=3.605）
　**差は +9.3 〜 +15.0円**。**A統計（ROI>100%）は 0/16**。
■ ★採用条件: **1（通るマスがある）✓ / 2（隣接と同じ向き・16マス）✓ /
　3（オッズで説明されない）✓ / 4（裾の検算）✓** → ★**4つとも満たした**（**この線で初めて**）。

■ ★★★★**ご提案の「推奨度の床」が、交絡を切った**
　★**4行すべてで、ズレの床を上げるとROIが単調に上がる**（**91.5→91.7→92.2→93.1** など）。
　★★**そして平均オッズは横ばい、むしろ微増**（**A=0.20の行で 10.5 → 10.7倍**）。
　⚠**(175)と(188)では同じ形が ρ(平均オッズ)=∓1.000 で「(88)の再発見」に落ちていた**。
　→ ★★**推奨度の床を入れたことで、オッズと切り離せた**。**事前登録に「見どころ」と書いた通り**。
　★**これがこの測定でいちばん価値のある発見**——**ユーザーの機構の読み
　　「いくらズレがあっても来ない馬は来ない」が、交絡の除去としても効いた**。

■ ★**予想と逆だったこと: 推奨度の床は「高いほど良い」ではない**
　**列で見ると 91.5 → 91.5 → 91.0 → 89.2%** と下がる。★**0.15〜0.20 が最良で、0.30 は悪化**。
　→ ★**床は「ゴミを落とす」ために要るが、上げすぎると本体も落ちる**。

■ ⚠⚠**それでも水準は届かない**
| | |
|---|---|
| **A統計（ROI>100%）** | ★**0/16マス**。**最良 93.6% の99%CIが [89.3, 98.0]** |
| ★**100%超の年** | **0/11**（13マス）／**1/11**（3マス） |
| 裾 | **上位3本が全払戻の 0.2〜0.4%**・**前後半で割れない** ＝★**健全** |
★**最良でも100%に 6.4pt 足りない**。**壊れているのではなく、足りない**。

■ 記述: **(112)の軸E との組み合わせ**（中央値で2分割）→ **88.0〜94.8% で明確な向きなし**。

■ ★★結論
　★**機構は確立した**——**16/16マス・4つの採用条件すべて・オッズ交絡から独立**。
　　**これは(177)(181)(184)の「モデルは同じオッズでも残余情報を持つ」を、
　　レース選別の形で最も強く確認したもの**。
　⚠**だが水準は 93.6% が上限**。**100%には 6.4pt 足りない**。
　⚠**経路は弱い（q側がML）ので「閉じた」とは書けない**（判定基準25）。
　★**書けるのは「推奨度とズレの絶対閾値で絞っても、1頭の札は100%に届かない」まで**。
