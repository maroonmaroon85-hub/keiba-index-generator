...
　⚠**環境差の上界は ±1.3pt**、**しかも向きは一方向**（**凍結時の方が0本の日は多い**）。
...
■ ★★★**この測定が副産物として見つけたこと（★私の測定より重要かもしれない）**
　⚠★★**(211)の189マスは、この環境では厳密に再現できない**。
　★**同じコード・同じデータでも、本数が 1,383 → 1,398 に動いた**（**+1.1%**）。
　★**原因は予測キャッシュ `data/cache/wf_pred.npz` が `.gitignore` されていること**
　　——**凍結時のモデルは当時のコンテナと共に消えた**。**版の固定も無い**
　　（**requirements も pyproject も無い**）。
　★**予測自体は run-to-run では完全に決定的**（**457,465行が一致・最大差 0.000e+00**）
　　なので、⚠**これは乱数ではなく★環境（ライブラリの版）の差である**。
　★★**影響: 前向きの検定には影響しない**（**これから貯める標本は今の環境で作られる**）。
　⚠**影響するのは「凍結した189マスの数字を後から検算したいとき」**。
　★**直すなら安い**: **`data/cache/wf_pred.npz` をコミットするか、版を固定する**。
　⚠★**ただしこれは ANA_RULE.md の凍結とは別の話なので、★勝手に直していない**。
