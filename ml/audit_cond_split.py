...
def main():
    ...
    print("  → " + ("★**条件を満たす**" if ok else "**条件を満たさない**"))

    print("\n── ⚠★事前登録5の judgment 文を訂正した（測ったあとに気づいた）──")
    print("  当初「条件を満たす＝**本命を外した買い目に意味がある**」と書いていたが**言い過ぎ**。")
    print("  ★**B（本命の枠が絡むか）は結果であって、発走前に選べない**。")
    print("  　「本命が飛んだレースだけ買う」は**実行できない**ので、D_condの内訳の")
    print("  　『飛んだ側の寄与が大きい』を買い方の指示として読んではいけない（(99)と同じ罠）。")
    print("  ★**正しく言えること**:")
    print(f"    ・我々の優位は**粗い構造（本命の枠が絡むか）にはほぼ無い**（{ab:+.4f}・CIが0をまたぐ）")
    print(f"    ・**細かい構造（どの枠組か）にほぼ全部ある**（{ac:+.4f}・11/11年）")
    print("    ・＝**枠連プールは『どの組か』の値付けが甘く、『本命が絡むか』は正しい**")
    print("  ★**次にやるべきこと（発走前に選べる形に直す）**:")
    print("    **買い目が本命の枠を含むか否か**は発走前に選べる。**そちらで測り直す**こと。")
    print("    ⚠ただし(89)④より、どう選んでも log(払戻率)+D は超えられない。**天井は動かない**。")
