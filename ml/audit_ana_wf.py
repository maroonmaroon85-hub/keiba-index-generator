...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-05・29,468レース・シード3本）—— **形は消えた。符号は残った**
────────────────────────────────────────────────────────────

■ ゲート1: 4帯とも通過
■ ★陽性対照: **三連複BOX上位4 = 81.9%**（**単一分割の84.5%より −2.6pt**）→ ★**立った**
　★★**(78)の「ウォークフォワードで2pt以上下がる」が再現した**。

■ ★★曲線（**ウォークフォワード**・複勝1点）
| 穴の程度 | **(a)水準** | 乱の水準 | ★**(b)差** | **(180)の差** | モデル1位率 |
|---|---|---|---|---|---|
| 1.5-3倍 | 89.6% | 88.8% | +0.8円 | +2.7円 | 45.8% |
| 3-5倍 | 83.1% | 81.1% | +2.1円 | +2.7円 | 40.0% |
| 5-8倍 | 80.5% | 78.9% | +1.6円 | +4.4円 | 7.1% |
| 8-12倍 | 82.9% | 77.3% | +5.6円 | +4.5円 | 0.7% |
| **12-20倍** | **84.9%** | 75.7% | **+9.2円** | +6.0円 | 0.1% |
| 20-35倍 | 81.8% | 75.0% | +6.9円 | +4.8円 | 0.0% |
| 35-60倍 | 78.2% | 73.7% | +4.5円 | +5.1円 | 0.0% |
| **60-120倍** | 73.6% | 72.0% | ⚠**+1.6円** | **+11.0円** | 0.0% |
| 120倍〜 | ⚠**58.8%** | 43.4% | **+15.5円** | +17.9円 | 0.0% |

■ ★★★主判定（**帰無は対応のある符号反転1,000回＝完全な randomization test**）
| | 実測 | 帰無99%点 | 判定 |
|---|---|---|---|
| **A 単調性 ρ** | **+0.517** | +0.867 | ⚠**超えない** |
| **B 正の帯数** | ★**9/9** | 8.0 | ★**超えた** |
★**両方が要る**（事前登録）→ ★**形は確認できない＝陰性**。

■ ★★**(180)の単調性を作っていたのは 60-120倍 だった**（**+11.0円 → +1.6円**）
　→ ★**判定基準39「3点で法則を書くな」**と
　　 ★**判定基準15「道具を新しくして構造が縮むなら、それは現象ではなく近似誤差」**の**両方の実演**。
　⚠**(180)の ρ=+0.950 は post-hoc に見た形で、事前登録した検定では ρ=+0.517**。

■ ★★残ったもの: **9/9 の符号が、正しい検定で通った**
　★**モデルの穴は、同じレース・同じオッズ帯の無作為な馬に、9帯すべてで勝つ**。
　★**事前登録つき・実運用と同じ手続き（毎年学習し直し）でも残った**。
　→ ★**(177)の「オッズを中和しても残余情報がある」が、判定基準6の宿題を果たした形で確認された**。
　⚠**ただし独立な標本ではない**（2017年以降は同じレース・同じ払戻）。**判定基準35**。

■ ⚠⚠**この線全体に効く重い副産物: 報告してきた水準は2〜3pt高すぎる**
　**BOX4 が 84.5% → 81.9%（−2.6pt）**。→ ★**(174)〜(180)の水準は実運用の手続きでは低く出る**。
　**(174)の最良87.6%も、実際は85%前後**。★**状況は良くなるのではなく悪くなる**。

■ ◇**post-hoc・未検定（拾わない）**: **12-20倍が「水準84.9%・差+9.2円」で目を引く**。
　⚠**主判定が通っていないので拾わない**（**(178)でプラセボが109.9%を出した直後**）。
　⚠**そして84.9%は100%になお15pt足りない**。**測るなら事前登録し直す**。
