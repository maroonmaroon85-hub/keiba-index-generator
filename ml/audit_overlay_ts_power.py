...
def main():
    ...
    if "確定" in rows:
        ...
        print(f"　→ ★★**①陽性対照は立ったか（CIが0を外すか）: "
              f"{'★立った' if stands else 'まだ立っていない'}**")
        if not stands:
            print("　⚠**①が立つまで、他の列（直前など）の符号を読んではいけない**（判定基準32）。")
            print("　　**真値の分かっている列を当てられない標本は、分からない列も当てられない**。")
        else:
            print("　★**①が立った。ここで初めて②（直前）を1回だけ読んでよい**。")
