...
def main():
    ...
    print(f"{'目的':<44}{'見分ける幅':>11}{'必要R':>9}{'開催日':>8}{'ぶれ幅':>16}")
    for name, sd, eff, one in (
        ...
        print(f"{name:<44}{eff:>11.4f}{n:>9,.0f}{n/34:>8.0f}"
              f"{f'{n*0.7:,.0f}〜{n*1.3:,.0f}':>16}")
    print("　※1開催日=約34レース（実測36Rのうち突き合わせできたのが27R＝歩留まり0.75）")

    print("\n" + "=" * 92)
    print("★読み方")
    print("  ★**まず20開催日（約660R）集めれば ①②が立つ**。**そこで(141)の生死が分かる**。")
    print("  ・**①が立たなければ何も読まない**（判定基準32）。**①は最初に確認すること**。")
    print("  ・**劣化の度合いまで測る（③）なら78開催日、精密に詰める（④）なら138開催日**。")
    print("  ⚠**①が立つまで(141)の117.3%は生死不明**。**閉じたとも生きているとも書かないこと**。")
