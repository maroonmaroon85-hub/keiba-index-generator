...
　★★**「4角で後方なのに上位で入った」は★平均に潰すと消える**。**そこが★記事の売り**。
　★**近走で何回それをやったかも数えた**（**レッドホットは近3走で2回**）。

■ ⚠⚠★★★★**(254sns) 枠連側が測って閉じた（2026-09-16）—— ★予想の根拠にはできない**

★**枠連側（`ml/audit_corner4.py` / ROI_MAP ◇176 / 8f687dc）が事前登録つきで測った**。
　★**観察は正しかった**: **corr(4角, 平均) = 0.9598**＝**ばらつきの約31%は平均で説明できない**。
　⚠⚠**だが「独立な情報があるか」は★否定された**:
| | 偏相関（logit(p) を差し引いた後・検証408,834行） | 99%CI |
|---|---|---|
| ★**本（4角残差）** | **+0.0008** | ⚠**[−0.0033, +0.0048] ＝ゼロを跨ぐ** |
| **対照（`last_passratio`＝★モデルに確実に入っている変数）** | **+0.0012** | ⚠**ゼロを跨ぐ** |
　★**本が対照を下回った**（**0.0008 < 0.0012**）。**年別の符号がそろった年 7/10＜必要8/10**。
　★★**結論: モデルが既に拾っている。この線は閉じた**（**偶数年学習・奇数年検証でも同じ**）。

⚠⚠★★**さらに、★符号が私の仮説と逆だった**——
　★**残差マイナス＝「押し上げた馬」のほうが次走で走る**。
　★**私の仮説は「4角で後方なのに上位＝過小評価」だったので、★機構ごと否定されている**。

■ ★★★★**だから、この行をどう使うか（★分けること）**
| 用途 | 可否 |
|---|---|
| ★**「前走は4角12番手から1着だった」と★描写する** | ★**可**（**事実。生データのとおり**） |
| ⚠**「だから今回も走る」と★根拠にする** | ⚠⚠**不可**（**測って否定された。★符号まで逆**） |
　★**有料で読ませるなら、ここを混ぜないこと**。**描写は売れるが、★否定済みの根拠は嘘になる**。

■ ⚠★★★**枠連側からの訂正（★私の警告が間違っていた）**
　★**私は「特徴量に足すと枠連117.3%((141))も別物になる」と書いたが、★誤り**。
　★**(141)の比の裾は★モデルを一切使わない**（**馬連の板→枠へ厳密集約 / 枠連の板**＝**板から板**）。
　→ ★**モデルを変えても117.3%は動かない**。**動くのは `waku_score` を使う運用側と(168)(169)(173)**。
　⚠**「枠連」と一括りにしたのが原因**——★**モデル経路と板経路は別物**。

■ ★★★★**いちばん重い教訓（★枠連側からの申し送り・こちらにも効く）**
　★**「Xは情報を足すか」を測るときの対照は、★いま動いている系そのものでなければならない**。
| 対照の取り方 | 4角残差の偏相関 |
|---|---|
| ⚠**前走の平均位置と log単勝 だけを差し引く**（★手で2つ選んだ代理） | **−0.0943** [−0.0965, −0.0921] ★**ゼロを外す** |
| ★**モデルの p に差し替える**（★動いている系そのもの） | **+0.0008** [−0.0033, +0.0048] ⚠**ゼロを跨ぐ** |
　⚠⚠**n=1,369,961 あったので、弱い対照では「自信を持ってゼロを外す」と書けてしまった**。
　★**実際はモデルに確実に入っている変数にすら負ける量だった**。
　★**特徴量を手で2つ3つ選んだ代理では足りない**——**判定基準に足すべき教訓**。
...
def main():
    ...
    print("⚠⚠★★**「4角後方から上位」を★予想の根拠にしないこと**（**(254sns)**）——")
    print("　★**枠連側が測って閉じた**: **モデルの p を差し引くと偏相関 +0.0008 でゼロを跨ぎ、")
    print("　　★モデルに確実に入っている対照変数(+0.0012)にすら負ける＝★既に拾われている**。")
    print("　⚠**しかも符号が逆**——**残差マイナス＝「押し上げた馬」の方が次走で走る**。")
    print("　★**描写として書くのは可。「だから走る」と書くのは★不可**。")
    return 0
