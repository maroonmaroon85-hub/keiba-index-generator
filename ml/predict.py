"""
⚠⚠⚠ このスクリプトは**否定済みの旧戦略**を実装しています。現状のまま実運用しないこと ⚠⚠⚠

下記の3点すべてが後の検証で覆されました（詳細は HANDOFF.md 冒頭と該当節）:
  1. **好ポケット(gap≥0.20 & 人気馬2-5倍 & ダート)による選抜** → (26)で**逆効果**と判明
     （無絞り込み64.3%に対しポケット56.4%＝-7.9pt悪化。純粋な過学習だった）
  2. **本線＝三連単 一軸マルチ×紐4** → (48)(54)(55)で三連複・枠連に明確に劣ると判明
  3. ★**ケリー配分の根拠が「好ポケット141.9%のエッジ」** → (25)(26)で**実際は56.4%**と判明。
     つまり下の KELLY 定数（f=2.6%＝1レース3,600円）は**存在しないエッジから計算された数字**で、
     これに従うと過剰ベットになる。**絶対にそのまま使わないこと**。

現時点の推奨（HANDOFF.md 冒頭参照）:
  ・モデル … **オッズを特徴に入れた top3目標モデル**（(45)(48)。このスクリプトが読む ml/model/ は
    train.py が作る「オッズなし・win目標」なので**そもそも別物**）
  ・買い方 … **枠連 軸枠×紐枠2（194円・ROI約84.5%）** ／ 配当重視なら三連複 BOX上位4（400円）
  ・絞り込み … (62)より**回収率は上がらない**。下位20%を避ける程度に留める
  ・ケリー配分 … 全券種でROI<100%なので**最適f=0%**。賭け金は「娯楽費として許容できる額」で決めるほかない

→ 実運用するには、オッズ入りtop3モデル＋枠連出力への書き換えが必要（未実施。HANDOFF「やり残し」参照）。

以下は旧仕様のままのコード。DG/DSのパース部分は再利用できる。
--------------------------------------------------------------------------------
