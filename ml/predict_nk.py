...
def main():
    ...
    # ⚠★**過去走に「対象日より後」の行が混じらないようにする**（2026-09-13に踏んだ）。
    # 　`last` は馬ごとの**最終行**を取るので、**後日の開催が入っていると未来を見る**。
    # 　★当日の運用では未来の行は存在しないので**これは no-op**。
    # 　⚠**効くのは過去日を再実行したとき**——**血統を埋めた効果を測ろうとして、
    # 　　8/30と9/6の比較が丸ごと無効になった**（履歴に9/12が入っていた）。
    fut = (d["date"] >= today).to_numpy()
    if fut.any():
        print(f"  ※対象日({today.date()})以降の過去走 {int(fut.sum()):,}行を除外（未来を見ないため）")
        d = d[~fut].reset_index(drop=True)
    ...
    # ⚠見出しは長く「紐枠2」のままだった（2026-09-13に直した）。**既定は紐1**——
    # 　(80)②でROI差は区別できないのにコストが倍かかると分かって紐1に変えている。
    # 　★`--partners` の値をそのまま出すので、二度と実態とずれない。
    print(f"\n===== {today.date()} 買い目（枠連 軸枠×紐枠{args.partners}"
          f" ／ 三連複 BOX上位4） =====")
