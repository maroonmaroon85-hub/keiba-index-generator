...
def main():
    ...
    print("\n── ★検出力（★この検証が何を言えて何を言えないか） ──")
    sd = diff.std(ddof=1)
    n99 = (zq(0.01) * sd / abs(m)) ** 2 if m else float("inf")
    print(f"  期待損失の差の標準偏差 {sd:.1f}円")
    print(f"  ★実測の効果 {m:+.2f}円/R を99%で検出するには約 {n99:,.0f}R 要る"
          f"（手元は {len(a):,}R）")
    print(f"  → **{'足りている' if n99 <= len(a) else '足りていない'}**。"
          f"{'' if n99 <= len(a) else '★この結論は「効果が無い」ではなく「まだ測れない」'}")
    # ★(81)は95%CIで主張した。同じ水準でどうなるかを必ず併記する（水準で結論が変わるため）
    se = sd / math.sqrt(len(diff))
    lo95, hi95 = m - zq(0.05) * se, m + zq(0.05) * se
    print(f"\n  ⚠**(81)は95%CIで主張した**ので同じ水準も出す: "
          f"L5−L2 = {m:+.3f}円/R 95%CI[{lo95:+.3f},{hi95:+.3f}]")
    print(f"  → 95%では{'**0を除外する**' if hi95 < 0 or lo95 > 0 else '0をまたぐ'}。"
          "★**水準で結論が変わる場合は、その事実こそが結論**")
