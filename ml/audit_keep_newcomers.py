...
━━━ ★★結果（2026-09-22・**事前登録どおり・条件は変えていない**）━━━

■ 内部対照: **落としている行 69,756 / 656,841 = 10.6%**。
　★**馬が増えたレース 3,740 / 25,755 = 14.5%**（増えた頭数は**中央1頭**）。
■ ゲート: **対照の 除外0%・紐1 ROI 85.3%**（既知 85.2%）→ ★一致。読んだ。
■ 主判定（除外40%・紐1 ／ ★**買うレース数は3腕とも同じ 15,453**）

| 腕 | ROI | 1Rあたり対応差 | 99%CI(Bonf) |
|---|---|---|---|
| **対照（現行）** | **87.2%** | — | — |
| 腕①（予測だけ緩める） | 87.0% | **−0.10円** | [−0.72, +0.52] ⚠ゼロを跨ぐ |
| 腕②（学習も緩める） | 86.6% | **−0.33円** | [−1.96, +1.30] ⚠ゼロを跨ぐ |
| **プラセボ（10種平均）** | — | **+0.08円** | 幅 [−0.42, +0.55]・SD 0.34 |

■ 年別の符号: **腕① 6/10 / 腕② 6/10**（必要 8 ＝ ceil(0.8×10)）。

★★★**どちらも3条件を満たさない＝落とす扱いを変えてもROIは動かない。運用は変えない**（判定基準40）。

★**読み方**
　1. ★**落としている行は10.6%で小さくない。増えるレースも14.5%ある**。
　　 ⚠**それでも対応差は両腕ともマイナス側**で、**プラセボ(+0.08円)にも届かない**。
　2. ⚠**腕②（学習も緩める）のほうが悪い**（86.6%）＝**新馬を学習に混ぜるとモデルが少し鈍る**。
　3. ★**事前に書いた予想「ROIは動かない」が当たった**。**理由も予想どおり**——
　　 **落ちる馬の大半は新馬で、`log_odds` は腕でも使えるので市場が既に値段を付けている**。
　4. ⚠★**9/19の件は「落とす扱い」のせいではなく「7/25のデータが無かった」せい**だった。
　　 **7/25は埋まったので再発は防げている**。★**逸話1件から扱いを変えなくて正解**（判定基準39）。

★★**今回の設計で効いたもの（★次もこうする）**
　1. ★**(170)の罠を構造で潰した**——**対照も腕も買うレース数を同じにした**ので、
　　 **(178)で出た「+7〜8円の人工物」が最初から出ていない**。**対応差が素直にゼロ近傍に落ちた**。
　2. ★**採用条件②を `ceil(0.8×年数)` にした**——**(178)の「5年しかないのに8/10を要求」が再発しなかった**。
