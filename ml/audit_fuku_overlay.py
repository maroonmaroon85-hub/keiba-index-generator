...
　　 **運用判定: ROIの99%CI下端 > 100%** かつ **年別で「91%を割る年が2つ以下」**。
　　 ⚠★**年の条件は結果を見る前に緩めた（2026-08-13・ユーザーの許容水準）**:
　　 　 「**毎年100%超えは要求しない。91%は許容する。40%とかは無理だけど**」。
　　 　 当初は「年8/11以上でプラス」と書いていた。**(141)を年変動だけで閉じかけた反省**。
　7. **★時間分割を最初から本判定に入れる**（(142)の教訓）。
　　 **2021-2026 だけでも CI下端 > 100%** を要求する。**全期間で出ても直近で出なければ採らない**。
　8. **プラセボ**: レース内で比を馬にランダム割り当てして同数買う。
　　 ⚠★**引かずに解析的に出す**（判定基準23の教訓）。無作為に k 頭選ぶときの期待払戻は
　　 　 **そのレースの全馬の平均払戻 × k** なので、**プラセボROI = Σ(全馬の払戻) / (100×頭数)**。
　　 　 **引くのは計算の無駄で、しかも標本誤差を足すだけ**。
    ...
    def run(which, mask):
        ...
        for t in THS:
            ...
            for i in np.where(mask)[0]:
                ...
                # ★プラセボは解析的に（無作為k頭の期待払戻 = 全馬平均 × k）
                pl_ret += float(d["pay"].mean()) * int(sel.sum())
                pl_cost += c
            if not prof:
                continue
            p = np.array(prof)
            ...
            out[t] = (len(p), nb, hit, ret / cost,
                      ...
                      pl_ret / max(pl_cost, 1.0), pos, len(set(ya.tolist())))
        return out
