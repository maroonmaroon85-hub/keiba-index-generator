...
────────────────────────────────────────────────────────────
★★★★ 結果（2026-09-16・**事前登録を2本コミットしたあとに走らせた**）
────────────────────────────────────────────────────────────
★★★**内部対照は2つとも立った**（軸1,398本・絞る前の P馬単M4点 ROI 112.9%）。

| | 本数 | ★**ROI** | 平均オッズ |
|---|---|---|---|
| **絞る前（biasが作れた分）** | 948 | 122.8% | 21.0倍 |
| ★★**絞って残した** | **469**（49.5%） | ★**149.6%** | 20.7倍 |
| **捨てた** | 479 | **96.6%** | 21.3倍 |
　⚠**既走3R未満で落ちた 450本**（**1,398 → 948**）。

■ ★★★★**主判定の返り値: 無作為と区別できない**
```
★無作為に469本選んだときのROI（2,000回）:
   中央 121.6% ／ ★99%点 173.9% ／ 範囲 [45.5, 190.7]
★★絞ったROI = 149.6%  →  ★★99%点を超えなかった（p = 0.167）
```
★★**「122.8% → 149.6%（+26.8pt）」は、★無作為に半分選ぶだけで普通に起きる幅だった**。
→ ★★**バイアスで絞ってもROIは動かない。★この線は閉じる**。

■ ★★**プラセボが無ければ騙されていた（★これがこの測定の一番の収穫）**
　⚠**「絞る前122.8% → 絞って149.6%、捨てた側は96.6%」だけ見れば、劇的に見える**。
　★**だが無作為プラセボの分布は [45.5, 190.7]**。★**149.6%はその中央付近（p=0.167）**。
　→ ★**枠連側が (178)② で踏んだのと同じ形を、★プラセボで回避できた**。

■ ★★ゲート2: ★**平均オッズ 残した20.7倍 − 捨てた21.3倍 = −0.6倍**
　★**(188)型のオッズ交絡は小さい**。⚠**交絡が無くても効果も無かった**。

■ ★**予想は当たった（★16回目の予想・当たり）**　「超えないと見る」→ そのとおり。

■ ⚠★★★**私の誤り: 判定基準37の★10回目・★同じ罠**
　★**初版の対照に 1,383本の 113.4% を書き、1,398本の実測 112.9% で落ちた**。
　⚠★**112.9% は (238) で私自身が測った値で、その訂正コミットに自分で書いていた**。
　★**対策を1つ足した**: ★★**凍結値を対照に書くときは、
　　★その値を「どの母集団で測ったか」を必ず併記する**。
