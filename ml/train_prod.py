...
実運用に使うのは検証で最良だった構成:
  ・目標  … **top3**(finish≤3)。**三連複は上位3頭で決まるため正しい**((48)(55))。
  　⚠★**枠連については、この理由付けは誤り**（2026-08-30・(168)で判明）。
  　　**枠連は1着と2着の枠だけで決まる。3着は無関係**。**(48)が比べたのは win vs top3 で、
  　　top2 は一度も候補に入っていなかった**。`waku_score` も **2*Pa*Pb という上位2頭の式**を
  　　**top3で学習した確率**に当てており、二重にずれている。
  　★**(168)で実測した**: 目標を絞ると**枠連 86.7→88.0(top2)→88.6%(win)** と単調に上がり、
  　　**三連複 86.4→85.1→83.9%** と単調に下がる（**目標が効いている証拠**）。
  　⚠**だが対応のある差は top2−top3 で +0.5円・99%CI[−3.1,+4.2] ＝検出できない**。
  　→ ★**目標は top3 のまま置く**。**理由は「正しいから」ではなく「変える根拠が無いから」**。
  　　 **この区別を残す**——**次に誰かが同じ違和感を持ったとき、測り直さずに済むように**。
