...
★★★実行済みの結果（2026-08-16・42,161レース・2015年以降）
| q_poolの作り方 | 全体D | 0-1.6帯 | 99%CI | 実測ROI |
|---|---|---|---|---|
| 下限そのまま | +0.0095 | +0.0570 | [+0.0367,+0.0773] | 94.4% |
| ★下限をΣ=3に | +0.0045 | **+0.1666** | [+0.1362,+0.1969] | 94.4% |
| ⚠中点をΣ=3に | +0.0102 | **+0.9339** | [+0.7603,+1.1075] | 94.4% ←**矛盾** |

★**事前登録4の整合性検査が発火した**。中点版は「必要量0.2231の4倍」と言いながら実測94.4%。
　→ ⚠**中点版の道具が壊れている**（複勝の板は[下限,上限]の範囲で、中点を採ると
　　 q_pool が小さくなりすぎ、log(q/q_pool) が上に飛ぶ）。**中点版は採らない**。
★**理屈上の正解は `q_pool,i = 0.8/オッズ_i`（Σ=3になる）**。複勝プールは3等分されるので
　`オッズ_i = 0.8·総額/(3·S_i)` → `S_i/総額 = 0.8/(3·オッズ_i)` → `Σ 1/オッズ = 3.75`、
　`q_i = 3·S_i/総額 = 0.8/オッズ_i` → **Σq = 3**。
　⚠**下限を使うと q_pool が過大＝我々に不利側**。**主張するときはこちらを使う**（保守側）。

★**生き残った2通りとも「どの帯も必要量0.2231に届かない」**（最大 +0.1666・CI上端 +0.1969）。
　→ ★**(99)②の +0.2395 は観測の偏りだったと確定**。**事前登録5の予想はおおむね当たり**
　　 （「+0.01〜+0.03に落ち着く」→ 全体 +0.0045〜+0.0102。⚠帯別の最大は予想より高い）。

⚠⚠**ただしこれで「複勝は閉じた」と書いてはいけない**（判定基準29・30）。
　**「Dが必要量に届かない」は閉鎖の証明ではない**。**比の裾を測る必要がある**。
　→ ★**(151) `audit_fuku_tail.py` でやる**。**(146)は三連複プール経由**で測ったが、
　　 **本物の複勝の板で比の裾を測ったことは一度も無い**。

⚠★**このスクリプトは(145)でτが足されたときから壊れていた**（`top3_probs` の引数不一致）。
　**複勝の板が無くて実行されなかったので、2026-08-16まで誰も気づかなかった**。
...
def main():
    ...
    for r in races:
        ...
        p = probs(hs)
        l2, l3 = lam[yy]
        # ⚠audit_fuku_lbs.top3_probs は (p, tau, l2, l3)。**τ=1 が素のλ補正Harville**。
        #   (145)でτが足されたときに呼び出し側が更新されず、**ずっと落ちていた**
        #   （複勝の板が無くて実行されなかったので気づかなかった・2026-08-16に発見）。
