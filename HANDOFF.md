...
2. **その後 連系(払戻)データ**: 「◆買い目・オッズ一括出力メニュー」は**発走前オッズの全表（巨大）で不要**と判明。必要なのは**払戻(確定配当)だけ**＝軽い（`data/sample/payout-schema.md` 参照）。取得場所は成績/払戻側で**要調査**（1レース分サンプルを出して列確定）。連系の**確率(harville.ts)・購入シミュ(exotic-sim.ts)は完成・テスト済み**なので、払戻CSVのパーサ(`payout-parser.ts` 新規)を足せば回収率が出せる状態。
