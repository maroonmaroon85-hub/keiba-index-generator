...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-05）—— ⚠**ゲート0で停止。主判定は読んでいない**
────────────────────────────────────────────────────────────

■ ゲート1: 4帯とも通過（88.1 / 82.4 / 54.4 / 38.2 ＝ (174)(175)と同値）
■ 標本: 26,583レース / 366,754頭（⚠**(174)(175)と同一**）

■ ★記述2が狙いどおり効いた —— **`ratio` と 単勝EV(p·o) の順位相関 = +0.9647**
　★★**「比」は(45)のEVそのものだった**。**算数の予測が実測で確認された**。
　→ ★**「比で測り直す」は独立した枝ではない**。**(45)の再測定になる**。

■ ★記述1 —— **オッズ中和で「差・比・推奨度」は潰れるか**（ビン40分位の中で）
| 組 | 同じ十分位 | \|差\|≤1 |
|---|---|---|
| **gap ↔ ratio** | ★**86.4%** | ★**98.8%** |
| share ↔ gap | 70.2% | — |
| share ↔ ratio | 63.6% | 94.9% |
★**gap と ratio はほぼ同一**＝**「差か比か」という問いは消えた**。**share だけ少し残る**。

■ ⚠**ゲート0（中和の検算）が落ちた**
| ビン | 平均オッズ 1→10 | max/min | ρ(平均オッズ) | 判定 |
|---|---|---|---|---|
| 20分位 | 65.4 → 71.7倍 | 1.097 | +1.000 | ⚠落ちた |
| **40分位** | 66.4 → **69.4倍** | ★**1.045**（要<1.05 **✓**） | **+1.000**（要<0.6 **✗**） | ⚠**落ちた** |

■ ★★★**判定基準37を当てる: 落ちたのはゲートの定式化のほう**
　★**ρ は尺度を持たない量なので、ビンをどれだけ細かくしても +1.000 のまま**——
　　**ビン内で「モデル評価が高い馬」はどんなに狭い帯でも低オッズ側に僅かに寄る**ので、
　　**精緻化しても ρ は下がらない**。→ ★★**どんな統制でも達成できない条件だった＝設計ミス**。
　★**大きさの条件(max/min<1.05)は 1.045 で満たしている**。
　→ ★**(175)の教訓を条件に変換したのは正しかったが、量ではなく形で書いたのが誤り**
　　（**判定基準32補足/(153)「許容幅はその量の測定誤差で決める。適当に置かない」の別形**）。

■ ⚠⚠**正直に記録する: ここから先は post-hoc になる**
　★**記述として出した20分位で、効果の大きさをもう見てしまった**
　　（**第10−第1 = +13.1円 / ρ(ROI)=+1.000**）。**40分位の表も見えている**。
　→ ★**ゲートを書き直すなら、それは結果を見た後の再定式化である**。**消さずに書く**。

■ ★見えている範囲で最も重要な数字（**ただし2点だけ**）
| ビン | 残ったオッズの幅 | 複勝ROI 1→10 | **ROIの幅** |
|---|---|---|---|
| 20分位 | **+9.7%** | 67.9 → 80.9% | **13.0pt** |
| 40分位 | ★**+4.5%**（半分） | 65.9 → 81.5% | ★**15.6pt**（**縮んでいない**） |
★★**交絡を半分に減らしたのに効果は縮まず、むしろ増えた**。
　→ ★**判定基準15「道具を新しくするたびに構造が縮んでいくなら、それは現象ではなく近似誤差」の逆向き**。
　⚠⚠**ただし2点だけ**。**判定基準39「3点で法則を書くな」に照らせば、2点で読むのはもっと悪い**。

■ ★次にやること（→ (177)・**ユーザーの判断待ち**）
　★**ρ を捨て、「精緻化の応答」そのものを判定にする**:
　　**NBIN = 10 / 20 / 40 / 80 の4点で回し、「残ったオッズの幅」に対して「効果(円)」を回帰**。
　　**交絡ならオッズの幅→0 で効果も→0 に外挿されるはず**。
　　★**判定 = 外挿した切片の99%CIが0を含むなら交絡と区別できない＝陰性**。
　★**falsifiable**: **10分位と80分位はまだ見ていない**。**80分位で効果が縮めばこの案は落ちる**。
　★**ゲート0は max/min < 1.05 のみに直す**（**大きさの条件だけ残す**）。
　⚠**post-hoc であることを(177)の登録の冒頭に明記すること**。
