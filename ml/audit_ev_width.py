...
★★**追記（同日・ユーザーの問い「対象レースはどう選んでいる？枠連はかためのレースを
　選んでいるからそこに固執しないほうがいい」）**
　★**答え: 選んでいない**。**(173)は検証期間の全26,918レース**で、`waku_score`の除外も
　　運用の選択も入っていない。**「買う率50.6%」は規則が+EVの組を見つけた割合**であって選別ではない。
　★**その上で、112.9%がどのレース種別に住んでいるかを測った**（2017年以降28,085レース・選別なし）:
| 荒れ度（市場エントロピー）五分位 | 発火率 | 平均点数 | ROI | 95%CI |
|---|---|---|---|---|
| **Q1 最も堅い** | 52.6% | 2.25 | ★**97.1%** | [75.5,118.6] |
| Q2 | 49.0% | 2.04 | **125.8%** | [102.1,149.4] |
| Q3 | 49.2% | 1.91 | 123.0% | [98.4,147.5] |
| Q4 | 49.9% | 1.90 | 115.9% | [88.6,143.2] |
| Q5 最も荒れる | 50.8% | 1.97 | 121.5% | [98.0,145.0] |
（1番人気の強さで切ると **Q5 最も断然 105.0%** が2番目に悪く、Q3 125.3% が最良）
★★**堅いレースがいちばん悪い**。**エッジは中間〜荒れ側に寄る**。⚠**CIは全部重なるので差は未確立**。
　正確には「**レース種別におおむね一様、強いて言えば堅いレースで弱い**」。

★★★**運用への含意（重要・忘れないこと）**: **運用は `waku_score` 下位40%を除外＝
　「確率が分散しているレース」を落としている**。⚠**比の裾のエッジはまさにそこに寄っている**。
　→ ★**(148)で比の裾が張れる時点でも成立したとき、運用の「堅いレースを選ぶ」選択を
　　そのまま持ち込んではいけない**。**別の賭けなので、選択も測り直す**。
⚠**混同しないこと**: **運用の枠連（MLで軸・紐）と比の裾（板から+EVの組を全部）は別の賭け**。
　**運用の除外は運用の賭けに対して測って有効**（(139)）。**片方の選択規則を他方に流用する根拠は無い**。
