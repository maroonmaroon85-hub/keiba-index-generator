...
────────────────────────────────────────────────────────────
★★★ 実測（2026-09-06・29,465レース）—— **両方とも届かない。だが内容は濃い**
────────────────────────────────────────────────────────────

■ ゲート1 4帯とも通過 ／ ゲート板 復元R 0.8009 ／ 陽性対照 BOX4 81.9% → 全部立った
■ ★★内部対照: **10/10マスで再現**（**家族Bの8マスは差 ±0.0pt**）→ **装置は完全に同一**

■ ★★家族A: **ズレ床を延長するとROIは上がり続ける。だが標本が減るほうが速い**
...
★**93.0 → 96.3% まで伸びた**（**(189)の93.6%を超え、この線の複勝の最高値**）。
⚠**だが標本が減るほうが速い**——**0.15で買うR 21.5%・CIの上端がやっと100%を跨ぐだけ**、
　**0.20で 6.6%（1,949R）・CI±8.7pt に広がり、ROIも 96.3 → 95.5% と下がる**。
→ ★**判定基準5「絞ると測れなくなる」がそのまま出た**。**100%超は 0/10マス**。
⚠★**平均オッズが 8.2 → 5.9倍 と下がる**ので、**(175)(188)の交絡が戻ってきている可能性がある**
　（**(189)の格子内では横ばいだった**）。**そこも延長の限界**。

■ ★★★家族B: **紐の順序で大きな差が出た**（**現行との対応差[円]**）
...
★★★**「紐もズレ順」にすると馬連が 89.0 → 95.3%（+6.3円）**。
⚠**だが三連複・三連単では 68.1% / 60.7% に大暴落**。
★**機構は明快**——**馬連は「2頭とも来ればいい」ので、ズレの大きい馬を相手にしても成立する。
　三連複・三連単は「3頭全部」なので、来にくい馬を2頭も入れると崩壊する**。
⚠**Bonferroniを通ったのは三連複の −26.0円（悪化）だけ**。**馬連の +6.3円 は通らない**。
★**人気順の紐は全券種で現行と同等かわずかに悪い**（−0.2 〜 −4.9円）＝
　★**(69)が要求する「人気順の対照」を、紐側でも初めて満たした**。

■ ★★結論
　★**ズレ床を延長しても、紐の順序を変えても、100%には届かない**（**34比較で0マス**）。
　★**ただし2つとも「効く方向」は見えた**——
　　**ズレ床は 96.3% まで伸び**、**紐をズレ順にすると馬連が +6.3円**。
　⚠**どちらも標本か多重性の壁に当たっている**。
　⚠**経路は弱いので「閉じた」とは書けない**（判定基準25）。

■ ◇**post-hoc・未検定（拾わない）**: ★**馬連 × 紐もズレ順 が 94.7 / 95.3%**（2絞りとも）。
　⚠**Bonferroniを通っていない**（+5.2 / +6.3円 に対し必要は z=3.620）。
　★**だが2つの絞りで同じ向き・同じ大きさ**なので、**事前登録し直す価値はある**。
　⚠**それでも 95.3% は100%に4.7pt足りない**。
