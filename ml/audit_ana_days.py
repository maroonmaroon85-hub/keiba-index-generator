...
■ ★★★★**(244sns) SNS版の結果（実測済み・2026-09-13）**
　★**条件: ズレ下限 0.10（(240sns)で採用）＋ 障害戦を除外（(243sns)）**
　⚠**内部対照は(231)の値なので落ちる。★条件を変えたのだから当然で、経路の誤りではない**。

**★★投稿カレンダーが根本的に変わる**
| | **(231) ズレ0.15・障害込み** | ★**(244sns) ズレ0.10・障害除外** |
|---|---|---|
| ★**該当0本の開催日** | ⚠**39.1%** | ★★**4.5%** |
| **1日あたりの本数** | 1.22本 | ★**4.39本** |
| **3本以上の日** | 15.8% | ★**69.1%** |
| ★**複勝が当たる日** | 23.7% | ★★**58.0%** |
| ★**連続で当たらない最大** | ⚠**35開催日** | ★★**10開催日** |
　★★**「2ヶ月当たらない期間が正常」という★この設計最大の弱点が、ほぼ消える**。
　⚠**代償はROI**（**複勝 101.5% → 94.2%**・(240sns)）。★**利用者はそれを承知で選んでいる**
　　（**「90%あれば上出来」「買う買わないはフォロワーに任せる」2026-09-12**）。

**★ゲート2: 外れの連は★ただのばらつき**
| 対照 | 最大 | 95%区間 | ★**実測以上の割合** |
|---|---|---|---|
| ⚠**対照1（素朴・間違い）** | 7.5日 | 5〜11日 | **0.097** |
| ★**対照2（正しい）** | **10.2日** | 7〜16日 | ★**0.575** |
　★**実測10日は対照2の平均10.2日とほぼ同じ**＝★**当たりは日に固まっていない**。
　⚠**(231)のときと違い、★対照1でも区別がつかない**（0.097）——
　　**0本の日が4.5%しかないので、★対照1の欠陥（0本の日に的中を配る）がほとんど効かないため**。


■ ★★★★**(247sns) ズレ下限の3水準を日別で並べる（実測済み・2026-09-15）**
　★**条件: 障害戦を除外（(243sns)）・下限だけを動かす**。⚠**内部対照は(231)の値なので落ちる（正常）**。

| | ★**0.15**（穴馬側と同じ） | ★**0.10**（現行） | ★**0.08** |
|---|---|---|---|
| **該当（11年）** | **1,321** | **5,015** | **8,094** |
| ★**1日あたり** | **1.16本** | **4.39本** | ★**7.09本** |
| ⚠★**該当0本の開催日** | ⚠**41.4%** | ★**4.5%** | ★★**0.8%** |
| **3本以上の日** | 14.6% | 69.1% | ★**91.1%** |
| ★★**複勝が当たる日** | **23.2%** | ★**58.0%** | ★★**75.3%** |
| ★**連続で当たらない最大** | ⚠**35開催日** | **10開催日** | ★★**5開催日** |
| 　★**同・正しい対照の最大** | **43.2日**[27〜73] | **10.2日**[7〜16] | **6.0日**[4〜9] |
| ⚠**複勝ROI**（(240sns)・障害込み） | ★**101.5%** | ⚠**94.2%** | ⚠**92.3%** |
| **◎の中央オッズ** | 15.8倍 | **16.4倍** | **16.5倍** |
| **複勝的中率** | 23.5% | **21.7%** | **21.6%** |

★★★**どの水準でも、連続外れは★正しい対照と区別がつかない**——**ばらつきの範囲**。
★★**◎の性格（中央オッズ・的中率）は3水準でほとんど動かない**。★**動くのは本数とROIだけ**。

■ ★★★**読み取れること**
　★**0.15→0.10 の一段目が、★見送り問題を解いた**（**41.4% → 4.5%**）。
　⚠★**0.10→0.08 の二段目で解ける問題は、もう残っていない**
　　（**該当0本 4.5%→0.8% は、★月に1度あるかないかの差**）。
　★**得られるのは1日あたりの本数**（**4.39→7.09本**）。⚠**対価はROI −1.9pt**。
　⚠⚠★**そして「絞っている」という看板が弱まる**——
　　**1日24レース中、0.10は4.4本(18%)、0.08は7.1本(★30%)に印を打つ**。
　　★**3レースに1レースは、もう「選んでいる」とは言いにくい**。
...
def main():
    ...
    if tuned:
        print(f"\n　★**条件を変えているので、本数({tot:,})が1,398と違うのは当然**"
              "（**(231)は ズレ0.15・障害込み**）")
    else:
