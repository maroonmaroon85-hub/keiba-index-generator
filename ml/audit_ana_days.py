...
# ★本数は「凍結値(1,383)〜当環境の実測値(1,398)」の範囲を対照にする（★上の訂正を読むこと）
N_LO, N_HI = 1383, 1398
ENV_GAP = N_HI - N_LO      # ★日別の答えに付ける誤差の上界
KNOWN_HIT, KNOWN_ODMED = 23.4, 15.9
...
def main():
    ...
    ok = [("該当レース総数", tot, float(N_LO), N_HI - N_LO, N_LO <= tot <= N_HI),
    ...
    for nm, got, want, tol, good in ok:
        rng = f"{N_LO:,}〜{N_HI:,}" if nm.startswith("該当") else f"{want:.1f} (±{tol})"
    ...
    # ★★環境差の上界（★訂正で約束した分）: 15本のレースは高々15日しか動かせない
    print(f"\n　⚠★**環境差の上界**: **凍結時(1,383本)との差は {ENV_GAP} 本**"
          f"　→ ★**0本の日の割合は 高々 "
          f"±{100*ENV_GAP/max(ma.sum(),1):.1f}pt (a) / ±{100*ENV_GAP/max(mb.sum(),1):.1f}pt (b)"
          f" しか動かない**")
    print("　★**しかも動く向きは一方向**——**凍結時の方が該当が15本少ない＝"
          "★0本の日は凍結時の方が多い（この測定は下限を出している）**")
