...
■ ★★★★**(233) の結果（実測済み・2026-09-09）**
　★**既存の行は全部そのまま再現した**（**G馬単M4点100% / Q三連単A4点50.0% / P馬単M4点100% /
　　X三連単A3点83.3% / X三連単A4点77.8% / P三連単A3点100% / 紐 P100・G100・Q75・X100**）
　　＝★**私の追記が既存の測定を1つも動かしていないことの対照**。
| 買い目 | 本数 | ★**買い目そのものの一致率** |
|---|---|---|
| ★**P馬連2点** | 6 | ★★**100.0%** |
| ⚠**Q馬連2点** | 6 | ⚠**66.7%** |
　★★**ゲート2の答え: PとQは同率ではなかった**（**100.0% vs 66.7%**）
　　→ ★**紐をQからPに差し替えた判断は、この物差しでは支持される**。
　★**Q馬連2点は朝9時に買い目の3分の1が入れ替わる**——**Q三連単A4点の50.0%と同じ向き**。
　★**予想は2つとも当たった**（判定基準24・★このセッション3回目）。

■ ⚠⚠★★**それでも、この数字を強く読んではいけない（★結果より重要）**
　★**該当は6本しかない**。⚠**判定基準5「本数が2桁のマスは何も言えない」に照らせば、
　　★6本は1桁である**。★**「6本で崩れなかった」以上のことは言えない**。
　★★**しかも 確定版で軸が立つ9本のうち、朝9時でも立つのは6本（66.7%）**
　　——⚠**残り3本は軸そのものが消える**（**単勝10.0倍の線を割る等**）。
　　★**一致率100%は「立った6本の中では」という条件つきである**。
　⚠**pnとqpを確定版で固定しているので、これは依然★上限**（(220)の但し書き）。

■ ★★★★**(235) 追記＋結果（2026-09-09・★SNS側の見せ場を3点→4点に替えたため）**
　⚠★**ANA_RULE側セッションの指摘で、P三連単A3点が(229)で後半91.7%・縮み−60.0pt と判明**。
　★**SNS用の見せ場を P三連単A4点 に替えたので、その朝9時一致率を測る**。⚠**マスは増えない**。
　★**結果: P三連単A4点 = 100.0%（6本）**。★**既存の行は全部そのまま再現**。
　⚠**該当6本＝判定基準5に照らせば1桁。「6本で崩れなかった」以上のことは言えない**。
...
■ ★★★★**(228) の結果（実測済み・2026-09-08）—— ★紐XはQの弱点を大きく埋めた**
| 紐 | ★**上位2頭の一致率** |
|---|---|
| **P**（モデル順） | ★**100.0%** |
| **G**（ズレ順） | ★**100.0%** |
...
■ ⚠★**規則への影響（★正直に書く）**
　★**Xを候補に加えると、それは「189マスの中から後付けで選んだ5つ目」になる**。
　★**規則 `ANA_RULE.md` は 2026-09-07 に凍結済み**で、⚠**買い目を1つ増やすのは規則の変更**。
　★**ただし前向きの標本はまだゼロ**なので、**変えるなら今が最後**という状況は昨日と同じ。
　⚠**私は判断しない。利用者が決める**。

■ ★★★★**(233) 追記（2026-09-09・★SNS用A案の本命を測る）**
　★**利用者がA案（軸＋馬連＋三連単、複勝は抑え）を選び、紐をQからPに差し替えた**。
　★**差し替えの理由がまさにこの一致率**——**(228)でQの上位2頭は75.0%、
　　Q三連単A4点の買い目一致は50.0%**。⚠**利用者は朝9時に買うので、確定オッズのROIは
　　買い目が変わった時点で意味を失う**。
　★**測るもの: `P馬連2点` と `Q馬連2点` の買い目一致率**。
　　★**Qも並べて出す**——**「Pに替えた判断が正しかったか」は、Qと並べないと言えない**（★対照）。
　⚠★★**マスは1つも増えない**。**(232)で測った馬連2点に、物差しをもう1本当てるだけ**。
　■ ★★ゲート2（判定基準42）——**何を返せば「Pに替えた意味が無い」か**
　　★**PとQの一致率が同じなら、紐を替えた根拠はこの物差しには無い**。
　　★**Pが100%でQがそれを明確に下回れば、差し替えは支持される**。
　■ ⚠⚠★★**先に書いておく限界（★結果より重要）**
　　★**(220)の対象は data/odds_ts の396レース＝11開催日**。**この軸は1日0.6本**なので、
　　★**該当は6本前後しか出ない**（**G馬単M4点の「100%」は 6/6 だった**）。
　　⚠★**判定基準5: 本数が2桁のマスは何も言えない。★6本は1桁である**。
　　→ ★★**ここで100%が出ても「6本で崩れなかった」以上のことは言えない**。
　　★**私はこれを「確認」と呼び、「証明」とは呼ばない**。
　■ 予想（⚠**当てにしない**）
　　★**P馬連2点は100%**と見る（**P紐の上位2頭が100%なので、2点なら動く余地が無い**）。
　　★**Q馬連2点はそれを下回る**と見る（**Q上位2頭が75.0%のため**）。
...
def axis_check(ts):
    ...
    bet = {"G馬単M4点": [], "Q三連単A4点": [], "P馬単M4点": [],
           ...
           # ★★(233) SNS用A案の本命。⚠**マスは増やさない**（(232)で測ったマスに物差しを当てるだけ）
           "P馬連2点": [], "Q馬連2点": [],
           # ★★(235) P三連単A3点が(229)で後半100%割れ → 4点に替える判断のため
           "P三連単A4点": []}
