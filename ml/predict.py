...
**期待値の前提を誤解しないための注記**:
  ROI 84.5% は**長期的に15.5%ずつ減る**という意味。それでも**プラスにはならない**。
  「損失を小さくしつつ楽しむ」ための出力であり、増やすためのものではない。
  ※(69)訂正: **同じ買い方を人気順で並べても81.6%**出る。モデルの正味の寄与は**+2.9pt**で、
    ハイパーパラメータ次第では0になる。「市場を大きく上回る」とは考えないこと。
...
**未来レースで劣化する要素（正直に書いておく）**:
  ...
  5. **`_prior_mean` 系4特徴が予測時だけ全行NaN**（(69)⑤で判明）… `build_today` の今日の行は
     `finratio=NaN` なので `jockey_form` `trainer_form` `sire_season_lift` `damsire_season_lift` が
     欠損する（学習時のNaN率は 0%/0%/9.3%/15.8%）。実測影響は 軸99.8%一致・枠連ROI 84.5→84.4% で
     **ほぼ無い**（重要度0.02〜0.16%）ため放置している。直すなら `_prior_mean` 側を直すこと。
