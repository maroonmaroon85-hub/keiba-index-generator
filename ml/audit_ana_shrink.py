...
■ ★★★★**結果（実測済み・2026-09-09）—— ★穴馬は減っていない。減っているのはズレ**
...
| ⑤ ①かつ③ | 0.130 | 0.023 | 0.17倍 |
| ★★**⑦ 3条件すべて** | **0.130頭** | **0.023頭** | ★★**0.17倍** |
| ★**そのレースのズレの最大（平均）** | **0.145** | **0.103** | ★**0.71倍** |
　★**平均出走頭数も 14.4 → 13.7 でほぼ一定**。
　⚠★**穴馬（10倍以上）は1レースあたり約10頭で、11年間ほぼ変わっていない**。
　★**③と⑥が完全に同じ数値**＝**ズレ≥0.15の馬はほぼ全部 pn≥0.15 を満たす**
　　（**ズレが大きい馬はモデルが高く評価している馬なので当然**）。

■ ★★★**構造**
| | ★**意味** |
|---|---|
| ★**穴馬の供給は変わらない** | **市場の側は11年間ほぼ同じ** |
| ★★**モデルが市場に追いついた** | ★**walk-forwardで学習データが増え、予測が市場に近づいた** |
| ⚠★**この線は自然に閉じていく** | ★**ズレが縮み続ければ、該当はゼロに近づく** |
　★**(234)で立てた仮説が当たった**（**11回目にして4回目の的中**）。
　⚠★**買う判断に直結する**: ★**「該当が年32本」は現在の値で、止まる保証がない**。
　　**2016年比で0.17倍まで来ており、傾きはまだ下向き**。

■ ⚠★**この測定で私が踏んだ誤り（★判定基準37の7回目）**
　★**初版の内部対照に (234) の年別（合計1,383）を置いて落ちた**。
　★**1,383は「±20%の乱が引けたレース」だけ**で、**この測定は乱を使わないので全数1,398が正しい**。
　⚠**昨日 ANA_RULE.md に「1,398/1,383/1,356の3つの数」の注記を自分で書いた翌日に、同じ穴**。
　★**結果を読む前に直した**（判定基準32）。
